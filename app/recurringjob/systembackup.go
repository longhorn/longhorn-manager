...
func StartSystemBackupJob(job *Job, recurringJob *longhorn.RecurringJob) error {
	systemBackupJob, err := newSystemBackupJob(job, recurringJob)
	if err != nil {
...
func newSystemBackupJob(job *Job, recurringJob *longhorn.RecurringJob) (*SystemBackupJob, error) {
...
func (job *SystemBackupJob) run() (err error) {
	...
	err = job.waitForSystemBackupToStates(finalStates)
	if err != nil {
		return err
	}
