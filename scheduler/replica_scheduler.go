...
func (rcs *ReplicaScheduler) scheduleReplicaToDisk(replica *longhorn.Replica, diskCandidates map[string]*Disk) {
	disk, err := rcs.getDiskWithMostBalanceScore(diskCandidates, replica.Spec.VolumeSize)
	if err != nil {
		logrus.WithError(err).Warnf("Failed to select disk for replica %v using balance score, falling back to selecting disk with most usable storage", replica.Name)
	}
...
func (rcs *ReplicaScheduler) getDiskWithMostBalanceScore(candidateDisks map[string]*Disk, replicaSize int64) (*Disk, error) {
