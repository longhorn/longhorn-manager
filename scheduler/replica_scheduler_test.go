...
func newDiskScheduleForNodeDisk(node *longhorn.Node, name string) *longhorn.DiskSchedule {
	status := node.Status.DiskStatus[name]
	return &longhorn.DiskSchedule{
		ObjectMeta: metav1.ObjectMeta{
			Name:      status.DiskUUID,
			Namespace: node.Namespace,
		},
		Spec: longhorn.DiskScheduleSpec{
			NodeID:                  node.Name,
			Name:                    name,
			SchedulingReplicas:      make(map[string]int64),
			SchedulingBackingImages: make(map[string]int64),
		},
		Status: longhorn.DiskScheduleStatus{
			ScheduledReplicaStatus:      make(map[string]*longhorn.DiskScheduledResourcesStatus),
			ScheduledBackingImageStatus: make(map[string]*longhorn.DiskScheduledResourcesStatus),
			StorageScheduled:            0,
		},
	}
}

func updateDiskScheduleReplica(diskSchedule *longhorn.DiskSchedule, rName string, rStatus *longhorn.DiskScheduledResourcesStatus) {
	diskSchedule.Spec.SchedulingReplicas[rName] = rStatus.RequiredSize
	diskSchedule.Status.ScheduledReplicaStatus[rName] = rStatus
	updateDiskScheduleStorage(diskSchedule)
}

func updateDiskScheduleStorage(diskSchedule *longhorn.DiskSchedule) {
	var accSize int64 = 0
	for _, resource := range diskSchedule.Status.ScheduledReplicaStatus {
		accSize += resource.ScheduledSize
	}
	for _, resource := range diskSchedule.Status.ScheduledBackingImageStatus {
		accSize += resource.ScheduledSize
	}
	diskSchedule.Status.StorageScheduled = accSize
}
...
func (s *TestSuite) TestReplicaScheduler(c *C) {
	...
	diskSchedule1 := newDiskScheduleForNodeDisk(node1, getDiskID(TestNode1, "1"))
	...
	diskSchedule2 := newDiskScheduleForNodeDisk(node2, getDiskID(TestNode2, "1"))
	...
	diskSchedule3 := newDiskScheduleForNodeDisk(node3, getDiskID(TestNode3, "1"))
	nodes := map[string]*longhorn.Node{
		TestNode1: node1,
		TestNode2: node2,
		TestNode3: node3,
	...
	tc.nodes = nodes
	diskSchedules := map[string]*longhorn.DiskSchedule{
		diskSchedule1.Name: diskSchedule1,
		diskSchedule2.Name: diskSchedule2,
		diskSchedule3.Name: diskSchedule3,
	}
	tc.diskSchedules = diskSchedules
	expectedNodes := map[string]*longhorn.Node{
		TestNode1: node1,
	}
	...
	tc.nodes = nodes
	tc.diskSchedules = map[string]*longhorn.DiskSchedule{}
	expectedNodes = map[string]*longhorn.Node{}
	tc.expectedNodes = expectedNodes
	tc.err = false
	...
	diskSchedule11 := newDiskScheduleForNodeDisk(node1, getDiskID(TestNode1, "1"))
	diskSchedule12 := newDiskScheduleForNodeDisk(node1, getDiskID(TestNode1, "2"))
	...
	diskSchedule21 := newDiskScheduleForNodeDisk(node2, getDiskID(TestNode2, "1"))
	diskSchedule22 := newDiskScheduleForNodeDisk(node2, getDiskID(TestNode2, "2"))
	...
	tc.nodes = nodes
	diskSchedules = map[string]*longhorn.DiskSchedule{
		diskSchedule11.Name: diskSchedule11,
		diskSchedule12.Name: diskSchedule12,
		diskSchedule21.Name: diskSchedule21,
		diskSchedule22.Name: diskSchedule22,
	}
	tc.diskSchedules = diskSchedules
	expectedNodes = map[string]*longhorn.Node{
	...
	diskSchedule21 = newDiskScheduleForNodeDisk(node2, getDiskID(TestNode2, "1"))
	diskSchedule21.Status.StorageScheduled = TestDiskAvailableSize
	diskSchedule22 = newDiskScheduleForNodeDisk(node2, getDiskID(TestNode2, "2"))
	nodes = map[string]*longhorn.Node{
		TestNode1: node1,
		TestNode2: node2,
	...
	diskSchedule1 = newDiskScheduleForNodeDisk(node1, getDiskID(TestNode1, "1"))
	updateDiskScheduleReplica(diskSchedule1, alreadyScheduledReplica.Name, &longhorn.DiskScheduledResourcesStatus{
		RequiredSize:  TestVolumeSize,
		ScheduledSize: TestVolumeSize,
	...
	diskSchedule2 = newDiskScheduleForNodeDisk(node2, getDiskID(TestNode2, "1"))
	updateDiskScheduleReplica(diskSchedule2, alreadyScheduledReplica.Name, &longhorn.DiskScheduledResourcesStatus{
		RequiredSize:  TestVolumeSize,
		ScheduledSize: 0,
		Result:        longhorn.DiskScheduledResultRejected,
	})
	disk = newDisk(TestDefaultDataPath, true, 0)
	...
	diskSchedule3 = newDiskScheduleForNodeDisk(node3, getDiskID(TestNode3, "1"))
	updateDiskScheduleReplica(diskSchedule3, alreadyScheduledReplica.Name, &longhorn.DiskScheduledResourcesStatus{
		RequiredSize:  TestVolumeSize,
		ScheduledSize: 0,
		Result:        longhorn.DiskScheduledResultRejected,
	})
...
func generateFailedReplicaTestCase(
	...
	diskSchedule := newDiskScheduleForNodeDisk(node1, getDiskID(TestNode1, "1"))
	updateDiskScheduleReplica(diskSchedule, alreadyScheduledReplica.Name, &longhorn.DiskScheduledResourcesStatus{
		RequiredSize:  TestVolumeSize,
		ScheduledSize: TestVolumeSize,
		Result:        longhorn.DiskScheduledResultScheduled,
	})
	...
	tc.nodes = nodes
	diskSchedules := map[string]*longhorn.DiskSchedule{
		diskSchedule.Name: diskSchedule,
	}
	tc.diskSchedules = diskSchedules
	return
}
...
func generateBestEffortAutoBalanceScheduleTestCase() *ReplicaSchedulerTestCase {
	tc := &ReplicaSchedulerTestCase{
		...
		diskSchedules:      make(map[string]*longhorn.DiskSchedule),
		replicasToSchedule: map[string]struct{}{},
		firstNilReplica:    -1,
	}
	...
	addDisks := func(node *longhorn.Node, index int64, disk longhorn.DiskSpec, storageAvailable int64, hasReplica bool) (diskID string) {
		...
		diskSchedule := newDiskScheduleForNodeDisk(node, id)
		if hasReplica {
			replica := newReplicaForVolume(tc.volume)
			replica.Spec.NodeID = node.Name
			tc.allReplicas[replica.Name] = replica
			updateDiskScheduleReplica(diskSchedule, replica.Name, &longhorn.DiskScheduledResourcesStatus{
				RequiredSize:  TestVolumeSize,
				ScheduledSize: TestVolumeSize,
				Result:        longhorn.DiskScheduledResultScheduled,
			})
		}
		tc.diskSchedules[diskSchedule.Name] = diskSchedule
