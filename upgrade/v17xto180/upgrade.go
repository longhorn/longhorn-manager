...
func upgradeVolumes(namespace string, lhClient *lhclientset.Clientset, resourceMaps map[string]interface{}) (err error) {
	...
	for _, v := range volumesMap {
		...
		v.Spec.BackupTargetName = ensureBackupTargetSet(backupTargetName)
	}

	return nil
}

func ensureBackupTargetSet(backupTargetName string) string {
	if backupTargetName == "" {
...
func upgradeBackupVolumes(namespace string, lhClient *lhclientset.Clientset, resourceMaps map[string]interface{}) (err error) {
	...
	for _, bv := range backupVolumeMap {
		...
		bv.Spec.BackupTargetName = ensureBackupTargetSet(backupTargetName)

		if bv.Spec.VolumeName == "" {
			bv.Spec.VolumeName = bv.Name
		}
		bv.Labels = ensureLabel(bv.Labels, types.LonghornLabelBackupTarget, bv.Spec.BackupTargetName)
		bv.Labels = ensureLabel(bv.Labels, types.LonghornLabelBackupVolume, bv.Spec.VolumeName)
	}
...
func upgradeBackups(namespace string, lhClient *lhclientset.Clientset, resourceMaps map[string]interface{}) (err error) {
	...
	for _, b := range backupMap {
		if b.Status.State != longhorn.BackupStateCompleted {
			continue
		}
		...
		b.Labels = ensureLabel(b.Labels, types.LonghornLabelBackupTarget, backupTargetName)
	}
...
func upgradeBackupBackingImages(namespace string, lhClient *lhclientset.Clientset, resourceMaps map[string]interface{}) (err error) {
	...
	for _, bbi := range backupBackingImageMap {
		...
		bbi.Spec.BackupTargetName = ensureBackupTargetSet(backupTargetName)
		if bbi.Spec.BackingImage == "" {
			bbi.Spec.BackingImage = bbi.Status.BackingImage
		}
		bbi.Labels = ensureLabel(bbi.Labels, types.LonghornLabelBackupTarget, bbi.Spec.BackupTargetName)
		bbi.Labels = ensureLabel(bbi.Labels, types.LonghornLabelBackingImage, bbi.Spec.BackingImage)
	}
