...
func copyTaintTolerationCSISidecarSetting(namespace string, lhClient *lhclientset.Clientset) error {
	...
	if err != nil {
		...
		sCSI = &longhorn.Setting{
			ObjectMeta: metav1.ObjectMeta{
				Name: string(types.SettingNameCSISidecarComponentTaintToleration),
			},
			Value: s.Value,
		}
		_, err := lhClient.LonghornV1beta2().Settings(namespace).Create(context.TODO(), sCSI, metav1.CreateOptions{})
		return err
	}
	...
	_, err = lhClient.LonghornV1beta2().Settings(namespace).Update(context.TODO(), sCSI, metav1.UpdateOptions{FieldValidation: metav1.FieldValidationStrict})
	if err != nil {
		return err
	}
	return nil
...
func copyNodeSelectorCSISidecarSetting(namespace string, lhClient *lhclientset.Clientset) error {
	...
	if err != nil {
		...
		sCSI = &longhorn.Setting{
			ObjectMeta: metav1.ObjectMeta{
				Name: string(types.SettingNameSystemManagedCSISidecarComponentsNodeSelector),
			},
			Value: s.Value,
		}
		_, err := lhClient.LonghornV1beta2().Settings(namespace).Create(context.TODO(), sCSI, metav1.CreateOptions{})
