...
func doResourceUpgrade(namespace string, lhClient *lhclientset.Clientset, kubeClient *clientset.Clientset) (err error) {
	...
	if semver.Compare(lhVersionBeforeUpgrade, "v1.4.4") < 0 {
		logrus.Debugf("Walking through the upgrade path v1.4.3 to v1.4.4")
		if err := v143to144.UpgradeResources(namespace, lhClient, kubeClient); err != nil {
			return err
		}
	}
