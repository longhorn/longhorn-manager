...
import (
	...
	"github.com/longhorn/longhorn-manager/upgrade/v110xto1102"
...
func doResourceUpgrade(namespace string, lhClient *lhclientset.Clientset, kubeClient *clientset.Clientset) (err error) {
	...
	// When lhVersionBeforeUpgrade < v1.10.2, it is v1.10.x. The `CheckUpgradePath` method would have failed us out earlier if it was not v1.10.x.
	if semver.Compare(lhVersionBeforeUpgrade, "v1.10.2") < 0 {
		logrus.Info("Walking through the resource status upgrade path v1.10.x to v1.10.2")
		if err := v110xto1102.UpgradeResourcesStatus(namespace, lhClient, kubeClient, resourceMaps); err != nil {
			return err
		}
	}
