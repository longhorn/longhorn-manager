...
import (
	...
	"github.com/longhorn/longhorn-manager/upgrade/v19xto192"
	"github.com/longhorn/longhorn-manager/upgrade/v19xto195"
...
func doResourceUpgrade(namespace string, lhClient *lhclientset.Clientset, kubeClient *clientset.Clientset) (err error) {
	...
	// When lhVersionBeforeUpgrade < v1.9.2, it is v1.9.x. The `CheckUpgradePath` method would have failed us out earlier if it was not v1.9.x.
	if semver.Compare(lhVersionBeforeUpgrade, "v1.9.2") < 0 {
		logrus.Info("Walking through the resource upgrade path v1.9.x to v1.9.2")
		if err := v19xto192.UpgradeResources(namespace, lhClient, kubeClient, resourceMaps); err != nil {
			return err
		}
		forceResourceUpdate = true
	}
	...
	// When lhVersionBeforeUpgrade < v1.9.5, it is v1.9.x. The `CheckUpgradePath` method would have failed us out earlier if it was not v1.9.x.
	if semver.Compare(lhVersionBeforeUpgrade, "v1.9.5") < 0 {
		logrus.Info("Walking through the resource status upgrade path v1.9.x to v1.9.5")
		if err := v19xto195.UpgradeResourcesStatus(namespace, lhClient, kubeClient, resourceMaps); err != nil {
			return err
		}
	}
