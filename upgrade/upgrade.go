...
func Upgrade(kubeconfigPath, currentNodeID, managerImage string, enableUpgradeVersionCheck bool) error {
	...
	if err := upgradeutil.CheckUpgradePath(namespace, lhClient, nil, eventRecorder, enableUpgradeVersionCheck, ""); err != nil {
		return err
	}
...
func doResourceUpgrade(namespace string, lhClient *lhclientset.Clientset, kubeClient *clientset.Clientset) (err error) {
	...
	if semver.Compare(lhVersionBeforeUpgrade, "v1.12.1") < 0 {
		logrus.Info("Walking through the resource upgrade path v1.12.0 to v1.12.1")
		if err := v1120to1121.UpgradeResources(namespace, lhClient, kubeClient, resourceMaps); err != nil {
			return err
		}
	}
	if semver.Compare(lhVersionBeforeUpgrade, "v1.13.0") < 0 {
		logrus.Info("Walking through the resource upgrade path v1.12.x to v1.13.0")
		if err := v112xto1130.UpgradeResources(namespace, lhClient, kubeClient, resourceMaps); err != nil {
			return err
		}
	}
