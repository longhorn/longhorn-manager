...
func upgradeLonghornNodes(namespace string, lhClient *lhclientset.Clientset) (err error) {
	...
	for i, node := range nodeList.Items {
		...
		if updateRequired == true {
			if _, err := lhClient.LonghornV1beta1().Nodes(namespace).Update(context.TODO(), &nodeList.Items[i], metav1.UpdateOptions{}); err != nil {
				return err
			}
		}
	}
...
func upgradeInstanceManagers(namespace string, lhClient *lhclientset.Clientset) (err error) {
	...
	for i := range imList.Items {
		if err := upgradeLabelsForInstanceManager(&imList.Items[i], lhClient, namespace); err != nil {
			return err
		}
	}
	return nil
...
func upgradeShareManagers(namespace string, lhClient *lhclientset.Clientset) (err error) {
	...
	for i := range smList.Items {
		if err := upgradeLabelsForShareManager(&smList.Items[i], lhClient, namespace); err != nil {
			return err
		}
	}
	return nil
...
func upgradeEngineImages(namespace string, lhClient *lhclientset.Clientset) (err error) {
	...
	for i := range eiList.Items {
		if err := upgradeLabelsForEngineImage(&eiList.Items[i], lhClient, namespace); err != nil {
			return err
		}
	}
	return nil
...
func upgradeShareManagerPods(namespace string, lhClient *lhclientset.Clientset, kubeClient *clientset.Clientset) (err error) {
	...
	for i, pod := range smPods {
		...
		if err := upgradeShareManagerPodsLabels(&smPods[i], sm, kubeClient, namespace); err != nil {
			return err
		}
	}
	return nil
...
func upgradeInstanceManagerPods(namespace string, kubeClient *clientset.Clientset) (err error) {
	...
	for i := range imPods {
		if err := updateIMPodLastAppliedTolerationsAnnotation(&imPods[i], kubeClient, namespace); err != nil {
			return err
		}
...
func upgradeCSIDaemonSetsLabels(kubeClient *clientset.Clientset, namespace string) (err error) {
	...
	for i := range eiDaemonSetList.Items {
		dsList = append(dsList, &eiDaemonSetList.Items[i])
	}
...
func upgradeShareManagerServicesLabels(kubeClient *clientset.Clientset, namespace string) (err error) {
	...
	for i := range svList.Items {
		if err := upgradeLabelsForService(&svList.Items[i], kubeClient, namespace); err != nil {
			return err
		}
	}
	return nil
