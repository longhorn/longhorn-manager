...
func (sc *SettingController) getDaemonSetsWithIncorrectCNI(endpointNetworkForRWXVolume *longhorn.Setting) ([]*appsv1.DaemonSet, error) {
	...
	annotValue := ""
	if endpointNetworkForRWXVolume.Value != "" {
...
// syncEngineImage deploys engine image when v1 engine is enabled
func (sc *SettingController) syncEngineImage() error {
	...
	if _, err := sc.ds.GetEngineImageRO(engineImageName); err != nil {
