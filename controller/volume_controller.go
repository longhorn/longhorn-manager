...
func (c *VolumeController) cleanupCorruptedOrStaleReplicas(v *longhorn.Volume, rs map[string]*longhorn.Replica) error {
	...
	for _, r := range rs {
		...
		if v.Spec.BackendStoreDriver == longhorn.BackendStoreDriverTypeV1 {
			if shouldCleanUpFailedReplicaV1(r, v.Spec.StaleReplicaTimeout, safeAsLastReplicaCount, v.Spec.EngineImage) {
...
func (c *VolumeController) upgradeEngineForVolume(v *longhorn.Volume, es map[string]*longhorn.Engine, rs map[string]*longhorn.Replica) error {
	...
	newImage, err := c.getEngineImageRO(v.Spec.EngineImage)
	if err != nil {
...
func shouldCleanUpFailedReplicaV1(r *longhorn.Replica, staleReplicaTimeout, safeAsLastReplicaCount int,
	...
	if r.Spec.EngineImage != volumeCurrentImage {
		return true
	}
	return false
}
