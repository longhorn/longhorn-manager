...
func (c *VolumeController) upgradeEngineForVolume(v *longhorn.Volume, es map[string]*longhorn.Engine, rs map[string]*longhorn.Replica) error {
	...
	if types.IsDataEngineV1(v.Spec.DataEngine) {
		...
		if e.Spec.Image != v.Spec.Image && e.Spec.TargetNodeID != v.Spec.TargetNodeID {
			log.WithField("engine", e.Name).Infof("Updating instance manager image to %v and target node ID to %v", v.Spec.Image, v.Spec.TargetNodeID)
			e.Spec.Image = v.Spec.Image
		...
		if e.Status.CurrentState == longhorn.InstanceStateRunning && !e.Status.CurrentTargetConnected {
