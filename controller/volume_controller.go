...
func (c *VolumeController) reconcileVolumeCondition(v *longhorn.Volume, e *longhorn.Engine,
	...
	// Check for snapshot data corruption first
	var snapshotTotalSize int64
	var snapshotSizeParseErr error
	if v.Spec.SnapshotMaxSize != 0 {
		snapshotTotalSize, snapshotSizeParseErr = c.getSnapshotTotalSize(e, log)
	}

	if snapshotSizeParseErr != nil {
		v.Status.Conditions = types.SetCondition(v.Status.Conditions,
			longhorn.VolumeConditionTypeTooManySnapshots, longhorn.ConditionStatusTrue,
			longhorn.VolumeConditionReasonSnapshotDataCorruption,
			fmt.Sprintf("Snapshot data corruption detected: %v", snapshotSizeParseErr))
	} else {
...
func (c *VolumeController) getSnapshotTotalSize(e *longhorn.Engine, log *logrus.Entry) (int64, error) {
	...
	for _, snapshotInfo := range e.Status.Snapshots {
		...
		if err != nil {
			log.WithField("snapshot", snapshotInfo.Name).WithError(err).Errorf("Failed to parse snapshot size %v, snapshot data may be corrupted", snapshotInfo.Size)
