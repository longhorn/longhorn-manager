...
func (c *VolumeController) openVolumeDependentResources(v *longhorn.Volume, e *longhorn.Engine, rs map[string]*longhorn.Replica, log *logrus.Entry) error {
	...
	if types.IsDataEngineV1(v.Spec.DataEngine) {
		...
		if v.Spec.Image == v.Status.CurrentImage && v.Spec.TargetNodeIDForLiveUpgrade == v.Status.CurrentTargetNodeIDForLiveUpgrade {
			e.Spec.DesireState = longhorn.InstanceStateRunning
		}
	}
...
func (c *VolumeController) closeVolumeDependentResources(v *longhorn.Volume, e *longhorn.Engine, rs map[string]*longhorn.Replica) {
	...
	if e.Spec.DesireState != longhorn.InstanceStateStopped || e.Spec.NodeID != "" {
		...
		e.Spec.NodeID = ""
		e.Spec.TargetNodeIDForLiveUpgrade = ""
...
func (c *VolumeController) upgradeEngineForVolume(v *longhorn.Volume, es map[string]*longhorn.Engine, rs map[string]*longhorn.Replica) (err error) {
	...
	if types.IsDataEngineV1(v.Spec.DataEngine) {
		...
		if v.Spec.TargetNodeIDForLiveUpgrade != "" {
			if e.Spec.TargetNodeIDForLiveUpgrade != v.Spec.TargetNodeIDForLiveUpgrade {
				if e.Spec.Image != v.Spec.Image {
					log.Infof("Updating image from %s to %s for v2 data engine live upgrade", e.Spec.Image, v.Spec.Image)
					e.Spec.Image = v.Spec.Image
				}

				log.Infof("Updating target node from %s to %s for v2 data engine live upgrade", e.Spec.TargetNodeIDForLiveUpgrade, v.Spec.TargetNodeIDForLiveUpgrade)
				e.Spec.TargetNodeIDForLiveUpgrade = v.Spec.TargetNodeIDForLiveUpgrade
				return nil
			}

			if !e.Status.TargetInstanceReplacementCreated && e.Status.CurrentTargetNodeIDForLiveUpgrade != v.Spec.TargetNodeIDForLiveUpgrade {
				log.Debug("Waiting for target instance replacement to be created")
				return nil
			}

			if e.Status.CurrentTargetNodeIDForLiveUpgrade != v.Spec.TargetNodeIDForLiveUpgrade {
				if e.Status.CurrentState == longhorn.InstanceStateRunning {
			...
			if v.Status.CurrentTargetNodeIDForLiveUpgrade != v.Spec.TargetNodeIDForLiveUpgrade {
				v.Status.CurrentTargetNodeIDForLiveUpgrade = v.Spec.TargetNodeIDForLiveUpgrade
				return nil
			}
		}
	}
...
func isV2DataEngineLiveUpgradeCompleted(v *longhorn.Volume) bool {
	return v.Spec.TargetNodeIDForLiveUpgrade != "" &&
		v.Spec.Image == v.Status.CurrentImage &&
		v.Spec.TargetNodeIDForLiveUpgrade == v.Status.CurrentTargetNodeIDForLiveUpgrade &&
...
func (c *VolumeController) finishLiveEngineUpgrade(v *longhorn.Volume, e *longhorn.Engine, rs map[string]*longhorn.Replica, log *logrus.Entry) {
	...
	if v.Spec.TargetNodeIDForLiveUpgrade == "" {
		v.Status.CurrentTargetNodeIDForLiveUpgrade = ""
		e.Spec.TargetNodeIDForLiveUpgrade = ""
	}
}
...
func isVolumeUpgrading(v *longhorn.Volume) bool {
	...
	return imageNotUpdated || v.Spec.TargetNodeIDForLiveUpgrade != v.Status.CurrentTargetNodeIDForLiveUpgrade
}
