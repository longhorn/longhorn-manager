...
func (c *VolumeController) reconcileVolumeSize(v *longhorn.Volume, e *longhorn.Engine, rs map[string]*longhorn.Replica, efs map[string]*longhorn.EngineFrontend) error {
	...
	if e.Spec.VolumeSize == v.Spec.Size {
		...
		// Engine spec. Do not use EngineFrontend.CurrentSize here: detached or
		// reattaching V2 frontends legitimately report 0 or temporarily lag the
		// backend size, and treating that as expansion-incomplete can spuriously
		// trigger volume-expansion attachment loops for otherwise idle volumes.
