...
const (
	...
	BackingImageDataSourcePodNamePrefix    = "backing-image-data-source-"
)
...
func (c *BackingImageDataSourceController) Run(workers int, stopCh <-chan struct{}) {
	...
	if !cache.WaitForNamedCacheSync("longhorn backing image data source", stopCh, c.bidsStoreSynced, c.biStoreSynced, c.nStoreSynced, c.pStoreSynced) {
		return
	}
...
func (c *BackingImageDataSourceController) cleanup(bids *longhorn.BackingImageDataSource) (err error) {
	...
	pod, err := c.ds.GetPod(GetBackingImageDataSourcePodName(bids.Name))
	if err != nil {
...
func (c *BackingImageDataSourceController) syncBackingImageDataSourcePod(bids *longhorn.BackingImageDataSource) (err error) {
	...
	podName := GetBackingImageDataSourcePodName(bids.Name)
	...
	podNotReadyMessage := ""
	if pod == nil {
		podNotReadyMessage = fmt.Sprintf("pod doesn't exist")
		...
		podNotReadyMessage = fmt.Sprintf("pod is being deleted")
	} else {
	...
	if pod == nil {
		...
		isValidTypeForRetry := bids.Spec.SourceType == types.BackingImageDataSourceTypeDownload
		...
		if !newBackingImageDataSource {
			if !c.backoff.IsInBackOffSinceUpdate(bids.Name, time.Now()) {
				...
				log.Infof("Start to recreate pod for image data source %v when the backoff window passed", bids.Name)
			} else {
...
func (c *BackingImageDataSourceController) generateBackingImageDataSourcePodManifest(bids *longhorn.BackingImageDataSource) (*v1.Pod, error) {
	...
	for key, value := range bids.Spec.Parameters {
	...
	podSpec := &v1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:            GetBackingImageDataSourcePodName(bids.Name),
...
func isBackingImageDataSourcePod(obj interface{}) bool {
	...
	if pod.Labels[types.GetLonghornLabelComponentKey()] == types.LonghornLabelBackingImageDataSource {
		return true
	}
	return false
}
...
func (m *BackingImageDataSourceMonitor) sync() {
	...
	defer func() {
		if syncErr != nil {
			...
			if m.retryCount == engineapi.MaxMonitorRetryCount {
				close(m.stopCh)
				logrus.Warnf("Stop monitoring since monitor %v sync reaches the max retry count %v", m.Name, engineapi.MaxMonitorRetryCount)
				return
			}
		} else {
			m.retryCount = 0
	...
	if err != nil {
		if datastore.ErrorIsNotFound(err) {
			close(m.stopCh)
			logrus.Warnf("Stop monitoring since backing image data source %v is not found", m.Name)
			return
		}
		syncErr = errors.Wrapf(err, "failed to get backing image data source %v during monitor sync", m.Name)
		logrus.Error(syncErr)
		return
	}
	if bids.Status.OwnerID != m.controllerID {
		close(m.stopCh)
		logrus.Warnf("Stop monitoring since backing image data source %v owner %v is not the same as monitor current controller %v", m.Name, bids.Status.OwnerID, m.controllerID)
		return
	}
