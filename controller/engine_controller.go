...
func (ec *EngineController) findInstanceManagerAndIPs(obj interface{}) (im *longhorn.InstanceManager, initiatorIP string, targetIP string, err error) {
	...
	if e.Spec.TargetNodeIDForLiveUpgrade != "" {
		...
		if !e.Status.TargetInstanceReplacementCreated && e.Status.CurrentTargetNodeIDForLiveUpgrade == "" {
			im = targetIM
		}
	}
...
func (ec *EngineController) DeleteInstance(obj interface{}) (err error) {
	...
	if e.Status.CurrentTargetNodeIDForLiveUpgrade != "" {
...
func (ec *EngineController) SwitchOverTarget(obj interface{}) error {
	...
	log.Infof("Preparing to switch over target to node %v. Initiator instance port details: port=%v, targetPort=%v, standbyTargetPort=%v",
		e.Spec.TargetNodeIDForLiveUpgrade, initiatorInstance.Status.PortStart, initiatorInstance.Status.TargetPortStart, initiatorInstance.Status.StandbyTargetPortStart)
	...
	log.Infof("Preparing to switch over target to node %v. Target instance port details: port=%v, targetPort=%v, standbyTargetPort=%v",
		e.Spec.TargetNodeIDForLiveUpgrade, targetInstance.Status.PortStart, targetInstance.Status.TargetPortStart, targetInstance.Status.StandbyTargetPortStart)
...
func (ec *EngineController) getTargetInstanceManagerForSwitchOver(e *longhorn.Engine) (targetIM *longhorn.InstanceManager, err error) {
	if e.Spec.TargetNodeIDForLiveUpgrade != "" {
		targetIM, err = ec.ds.GetRunningInstanceManagerByNodeRO(e.Spec.TargetNodeIDForLiveUpgrade, e.Spec.DataEngine)
		if err != nil {
			return nil, errors.Wrapf(err, "failed to get instance manager on node %v for switching over target", e.Spec.TargetNodeIDForLiveUpgrade)
		}
...
func (ec *EngineController) DeleteTarget(obj interface{}) error {
	...
	if e.Spec.NodeID == e.Spec.TargetNodeIDForLiveUpgrade {
		targetNodeID = e.Status.CurrentTargetNodeIDForLiveUpgrade
	}
	if targetNodeID == "" {
...
func (ec *EngineController) RequireRemoteTargetInstance(obj interface{}) (bool, error) {
	...
	return e.Spec.TargetNodeIDForLiveUpgrade != "" && e.Spec.TargetNodeIDForLiveUpgrade != e.Spec.NodeID, nil
}
...
func (ec *EngineController) GetInstance(obj interface{}, isInstanceOnRemoteNode bool) (*longhorn.InstanceProcess, error) {
	...
	if isInstanceOnRemoteNode {
		nodeID = e.Spec.TargetNodeIDForLiveUpgrade
...
func (ec *EngineController) Upgrade(e *longhorn.Engine, log *logrus.Entry) (err error) {
	...
	if types.IsDataEngineV1(e.Spec.DataEngine) {
		...
		im, err = ec.ds.GetRunningInstanceManagerByNodeRO(e.Spec.TargetNodeIDForLiveUpgrade, longhorn.DataEngineTypeV2)
		if err != nil {
			return err
		}
		if im.Status.CurrentState != longhorn.InstanceManagerStateRunning {
