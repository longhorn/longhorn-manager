...
func (m *EngineMonitor) refresh(engine *longhorn.Engine) error {
	...
	if (types.IsDataEngineV1(engine.Spec.DataEngine) && cliAPIVersion >= engineapi.CLIAPIMinVersionForExistingEngineBeforeUpgrade) ||
		...
		frontendExpansionFailed := volumeInfo.LastExpansionError != ""
		publishSize, publishExpansionState, sizeConfirmed := resolveFrontendSizeReporting(sizeFollowsFrontend, frontendReported, frontendExpansionFailed, engine.Status.CurrentSize != 0)
...
// resolveFrontendSizeReporting decides how a v2 refresh treats the volume size it
// just read. sizeConfirmed is false when the frontend has to report the size but
// the proxy fell back to the engine or the frontend reported an expansion error,
// so an expansion must not be completed on it. publishSize still records the first
// size when none is known yet. publishExpansionState accepts a frontend-reported
// failure as terminal state while keeping the previous size.
func resolveFrontendSizeReporting(sizeFollowsFrontend, frontendReported, frontendExpansionFailed, haveCurrentSize bool) (publishSize, publishExpansionState, sizeConfirmed bool) {
	frontendStateReported := frontendReported || !sizeFollowsFrontend
	sizeConfirmed = !sizeFollowsFrontend || (frontendReported && !frontendExpansionFailed)
	publishSize = sizeConfirmed || !haveCurrentSize
	publishExpansionState = frontendStateReported
	return publishSize, publishExpansionState, sizeConfirmed
}
