...
func (c *SystemBackupController) WaitForBackingImageBackupToComplete(backupBackingImages map[string]*longhorn.BackupBackingImage, systemBackup *longhorn.SystemBackup) (err error) {
	log := getLoggerForSystemBackup(c.logger, systemBackup)
	...
	for range ticker.C {
		for name := range backupBackingImages {
			...
			case longhorn.BackupStateError:
				return errors.Wrapf(fmt.Errorf(backupBackingImage.Status.Error), "failed creating BackingImage backup %v", name)
			}
		}
		...
		if time.Since(startTime) > datastore.BackingImageBackupTimeout {
			return fmt.Errorf("timed out waiting for BackingImage backups to complete")
		}
	}
	// This should never be reached, return this error just in case.
	return fmt.Errorf("unexpected error: stopped waiting for BackingImage backups without completing, failing or timing out")
