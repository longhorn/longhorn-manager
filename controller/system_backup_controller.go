...
func (c *SystemBackupController) WaitForVolumeBackupToComplete(backups map[string]*longhorn.Backup, systemBackup *longhorn.SystemBackup) (err error) {
	...
	autoCleanupSnapshot, err := c.ds.GetSettingAsBool(types.SettingNameAutoCleanupRecurringJobBackupSnapshot)
	if err != nil {
		log.WithError(err).Warnf("Failed to get setting %v, fallback to disabled", types.SettingNameAutoCleanupRecurringJobBackupSnapshot)
