...
func (nc *NodeController) cleanupDiskSchedule(node *longhorn.Node) error {
	nodeDisks := map[string]bool{}
	if node.DeletionTimestamp.IsZero() && node.Status.DiskStatus != nil {
		for _, diskStatus := range node.Status.DiskStatus {
			nodeDisks[diskStatus.DiskUUID] = true
		}
	}

	diskScheduleMap, err := nc.ds.ListDiskSchedulesOnNode(node.Name)
	if err != nil && !datastore.ErrorIsNotFound(err) {
		return errors.Wrapf(err, "failed to list disk schedule on node %v for clean up", node.Name)
	}

	deleteErrs := multierr.NewMultiError()
	for _, diskSchedule := range diskScheduleMap {
		if !nodeDisks[diskSchedule.Name] {
			nc.logger.Infof("Clearning disk schedule %v (%q) on node %v", diskSchedule.Name, diskSchedule.Spec.Name, node.Name)
			if deleteErr := nc.ds.DeleteDiskSchedule(diskSchedule.Name); deleteErr != nil {
				deleteErrs.Append(fmt.Sprintf("%v (%v)", diskSchedule.Name, diskSchedule.Spec.Name), deleteErr)
			}
		}
	}

	if len(deleteErrs) > 0 {
		return fmt.Errorf("failed to delete disk schedules on node %v: %v", node.Name, deleteErrs.ErrorByReason("errors"))
	}
	return nil
}
