...
func (nc *NodeController) cleanupDisksBeforeNodeDeletion(node *longhorn.Node) error {
	...
	for diskName, diskInfo := range node.Spec.Disks {
		...
		diskStatus, ok := node.Status.DiskStatus[diskName]
		if !ok {
			nc.logger.Infof("Disk %s exists in spec but is missing from status. Skipping longhorn block disk cleanup for v2 data engine.", diskName)
			continue
		}

		if err := nc.deleteDisk(diskStatus.Type, diskStatus.DiskName, diskStatus.DiskUUID, diskStatus.DiskPath, string(diskStatus.DiskDriver)); err != nil {
			return errors.Wrapf(err, "failed to delete SPDK disk %v during node cleanup", diskStatus.DiskName)
		}
	}
