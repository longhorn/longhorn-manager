...
func (s *TestSuite) TestSyncEngineImageDaemonSetNodeSelectorSkipsUpdateWithAttachedVolume(c *C) {
	kubeClient := fake.NewSimpleClientset()                   // nolint: staticcheck
	lhClient := lhfake.NewSimpleClientset()                   // nolint: staticcheck
	extensionClient := apiextensionsfake.NewSimpleClientset() // nolint: staticcheck
	...
	setting, err := lhClient.LonghornV1beta2().Settings(TestNamespace).Create(
		context.TODO(), newSetting(string(types.SettingNameSystemManagedComponentsNodeSelector), "kubernetes.io/hostname:"+TestNode1), metav1.CreateOptions{})
	c.Assert(err, IsNil)
	err = settingIndexer.Add(setting)
	c.Assert(err, IsNil)
...
func (s *TestSuite) TestSyncEngineImageDaemonSetNodeSelectorUpdatesWhenVolumesDetached(c *C) {
	kubeClient := fake.NewSimpleClientset()                   // nolint: staticcheck
	lhClient := lhfake.NewSimpleClientset()                   // nolint: staticcheck
	extensionClient := apiextensionsfake.NewSimpleClientset() // nolint: staticcheck
	...
	err = ic.syncEngineImageDaemonSetNodeSelector(ds)
	c.Assert(err, IsNil)
	c.Assert(ds.Spec.Template.Spec.NodeSelector, DeepEquals, map[string]string{"kubernetes.io/hostname": TestNode1})
}
