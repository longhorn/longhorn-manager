...
func (sc *SnapshotConcurrentLimiter) canStartHeavyIOTask(engineClientProxy engineapi.EngineClientProxy, engine *longhorn.Engine, ds *datastore.DataStore, taskType snapshotHeavyTask) (bool, error) {
	...
	if err != nil {
		sc.logger.WithError(err).Debug("Failed to get snapshot purge concurrent limit setting, allowing by default")
		return true, nil
	}
	...
	for _, e := range engines {
		...
		sc.iterateReplicasForEngine(e, replicas, nodeLoad)
	}

	// Determine the nodes that *would be involved* if we start this engine’s task
	involvedNodes := map[string]int{}
	sc.iterateReplicasForEngine(engine, replicas, involvedNodes)
...
func (sc *SnapshotConcurrentLimiter) checkEngineBusy(engine *longhorn.Engine, engineClientProxy engineapi.EngineClientProxy, ds *datastore.DataStore) bool {
	...
	isCloning := sc.checkEngineCloning(engine, engineClientProxy, ds)
	return isCloning
}
...
// iterateReplicasForEngine applies a callback for each replica node of the given engine.
func (sc *SnapshotConcurrentLimiter) iterateReplicasForEngine(engine *longhorn.Engine, replicas map[string]*longhorn.Replica, nodeCounter map[string]int) {
