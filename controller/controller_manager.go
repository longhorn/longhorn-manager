...
func StartControllers(logger logrus.FieldLogger, stopCh chan struct{}, controllerID, serviceAccount, managerImage, kubeconfigPath, version string, proxyConnCounter util.Counter) (*datastore.DataStore, *WebsocketController, error) {
	...
	volumeController := NewVolumeController(logger, ds, scheme, kubeClient, namespace, controllerID, proxyConnCounter)
	...
	settingController := NewSettingController(logger, ds, scheme, kubeClient, namespace, controllerID, version)
	...
	backingImageController := NewBackingImageController(logger, ds, scheme, kubeClient, namespace, controllerID, serviceAccount)
	backingImageManagerController := NewBackingImageManagerController(logger, ds, scheme, kubeClient, namespace, controllerID, serviceAccount)
	backingImageDataSourceController := NewBackingImageDataSourceController(logger, ds, scheme, kubeClient, namespace, controllerID, serviceAccount, proxyConnCounter)
