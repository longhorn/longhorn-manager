...
func (imc *InstanceManagerController) handlePod(im *longhorn.InstanceManager) error {
	...
	log.Warnf("Deleting instance manager pod %v since one of the following conditions is met: "+
		"setting is not synced (%v), instances are running in the pod (%v), or the pod is deleted or not running (%v)",
		im.Name, !isSettingSynced, areInstancesRunningInPod, isPodDeletedOrNotRunning)
