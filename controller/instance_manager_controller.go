...
func (imc *InstanceManagerController) syncStatusWithPod(im *longhorn.InstanceManager) error {
	...
	if pod == nil {
		if im.Status.CurrentState == "" || im.Status.CurrentState == types.InstanceManagerStateStopped {
...
func (imc *InstanceManagerController) syncInstanceManagerAPIVersion(im *longhorn.InstanceManager) error {
	...
	if im.Status.CurrentState != types.InstanceManagerStateRunning && im.Status.CurrentState != types.InstanceManagerStateUnknown {
	...
	if im.Status.CurrentState == types.InstanceManagerStateRunning && im.Status.APIVersion == engineapi.UnknownInstanceManagerAPIVersion {
...
func (imc *InstanceManagerController) syncMonitor(im *longhorn.InstanceManager) error {
	...
	isMonitorRequired := im.Status.CurrentState == types.InstanceManagerStateRunning &&
