...
func (dsc *DiskScheduleController) reconcile(diskScheduleName string) (err error) {
	...
	if dsc.isDiskScheduleInitialing(diskSchedule) {
		...
		if err := dsc.syncFromAllocatedResources(diskSchedule); err != nil {
			return err
		}
	}
...
func (dsc *DiskScheduleController) syncFromAllocatedResources(diskSchedule *longhorn.DiskSchedule) error {
	...
	updateAllocation := func(specMap *map[string]int64, statusMap *map[string]*longhorn.DiskScheduledResourcesStatus, name string, size int64) {
		if *specMap == nil {
			*specMap = make(map[string]int64)
		}
		if *statusMap == nil {
			*statusMap = make(map[string]*longhorn.DiskScheduledResourcesStatus)
		}

		(*specMap)[name] = size
		(*statusMap)[name] = &longhorn.DiskScheduledResourcesStatus{
			Result:        longhorn.DiskScheduledResultScheduled,
			RequiredSize:  size,
			ScheduledSize: size,
		}
	}

	// sync from existing replicas
	replicas, replicaErr := dsc.ds.ListReplicasByDiskUUID(diskSchedule.Name)
	if replicaErr != nil {
		return errors.Wrapf(replicaErr, "failed to list existing replicas for disk schedule %v", diskSchedule.Name)
	}
	for _, replica := range replicas {
		if !isReplicaAllocatedToDisk(replica, diskSchedule) {
			continue
		}
		logger.Infof("sync from allocated replica %v for disk schedule %v", replica.Name, diskSchedule.Name)
		updateAllocation(&diskSchedule.Spec.SchedulingReplicas, &diskSchedule.Status.ScheduledReplicaStatus, replica.Name, replica.Spec.VolumeSize)
	}

	// sync from existing backing images
	bisMap, biErr := dsc.ds.GetCurrentDiskBackingImageMap()
	if biErr != nil {
		return errors.Wrapf(biErr, "failed to list existing backing image for disk schedule %v", diskSchedule.Name)
	}
	bims, bimErr := dsc.ds.ListBackingImageManagersByDiskUUID(diskSchedule.Name)
	if bimErr != nil {
		return errors.Wrapf(bimErr, "failed to list existing backing image manager for disk schedule %v", diskSchedule.Name)
	}
	for _, bi := range bisMap[diskSchedule.Name] {
		if !isBackingImageAllocatedToDisk(bi, bims, diskSchedule) {
			continue
		}
		logger.Infof("sync from allocated backing images %v for disk schedule %v", bi.Name, diskSchedule.Name)
		updateAllocation(&diskSchedule.Spec.SchedulingBackingImages, &diskSchedule.Status.ScheduledBackingImageStatus, bi.Name, bi.Status.Size)
	}
	return nil
}

func isReplicaAllocatedToDisk(replica *longhorn.Replica, diskSchedule *longhorn.DiskSchedule) bool {
	if !replica.DeletionTimestamp.IsZero() {
		return false
	}
	if replica.Spec.NodeID != diskSchedule.Spec.NodeID || replica.Spec.DiskID != diskSchedule.Name {
		return false
	}
	if replica.Spec.VolumeSize == 0 {
		return false
	}
	condition := types.GetCondition(replica.Status.Conditions, longhorn.ReplicaConditionTypeDiskAllocation)
	return condition.Status == longhorn.ConditionStatusTrue
}

func isBackingImageAllocatedToDisk(bi *longhorn.BackingImage, diskBims map[string]*longhorn.BackingImageManager, diskSchedule *longhorn.DiskSchedule) bool {
	if !bi.DeletionTimestamp.IsZero() {
		return false
	}
	if _, isDesiredDisk := bi.Spec.DiskFileSpecMap[diskSchedule.Name]; !isDesiredDisk {
		return false
	}
	if _, isSchedulingOnDisk := bi.Status.DiskFileStatusMap[diskSchedule.Name]; !isSchedulingOnDisk {
		return false
	}

	for _, bim := range diskBims {
		if _, exist := bim.Spec.BackingImages[bi.Name]; exist {
			return true
		}
	}
	return false
...
func (dsc *DiskScheduleController) syncResourceDeallocation(diskSchedule *longhorn.DiskSchedule) error {
	...
	err := cleanUpRequirement(diskSchedule.Spec.SchedulingReplicas, func(rName string) (bool, error) {
	...
	err = cleanUpRequirement(diskSchedule.Spec.SchedulingBackingImages, func(biName string) (bool, error) {
		...
		_, isDesiredDisk := bi.Spec.DiskFileSpecMap[biName]
		_, isSchedulingOnDisk := bi.Status.DiskFileStatusMap[biName]
		return !isDesiredDisk && !isSchedulingOnDisk, nil
	})
	if err != nil {
	...
	syncResourceStatus := func(specMap map[string]int64, statusMap map[string]*longhorn.DiskScheduledResourcesStatus) (scheduledSize int64) {
		...
		for resource, status := range statusMap {
			...
			if exist {
				scheduledSize += status.ScheduledSize
			} else {
	...
	scheduledReplicaSize := syncResourceStatus(diskSchedule.Spec.SchedulingReplicas, diskSchedule.Status.ScheduledReplicaStatus)
	scheduledBackingImageSize := syncResourceStatus(diskSchedule.Spec.SchedulingBackingImages, diskSchedule.Status.ScheduledBackingImageStatus)
...
func (dsc *DiskScheduleController) syncResourceAllocation(diskSchedule *longhorn.DiskSchedule) error {
	...
	resourceList := make([]resourceItem, 0, len(diskSchedule.Spec.SchedulingReplicas)+len(diskSchedule.Spec.SchedulingBackingImages))
	for name := range diskSchedule.Spec.SchedulingReplicas {
		resourceList = append(resourceList, resourceItem{
			kind:      "Replica",
			name:      name,
			specMap:   diskSchedule.Spec.SchedulingReplicas,
			statusMap: &diskSchedule.Status.ScheduledReplicaStatus,
		})
	}
	for name := range diskSchedule.Spec.SchedulingBackingImages {
		resourceList = append(resourceList, resourceItem{
			kind:      "BackingImage",
			name:      name,
			specMap:   diskSchedule.Spec.SchedulingBackingImages,
			statusMap: &diskSchedule.Status.ScheduledBackingImageStatus,
		})
	}
	...
	for _, resource := range resourceList {
		...
		if *resource.statusMap == nil {
			*resource.statusMap = make(map[string]*longhorn.DiskScheduledResourcesStatus)
		}
		...
		if requiredSize == status.ScheduledSize {
			// already reconciled
			continue
		}
		schedulableMaximum := diskStatus.StorageMaximum - diskSpec.StorageReserved
		result, updateSize, updateAccSize := dsc.reconcileResourceAllocation(resourceLogger, accScheduledSize, schedulableMaximum, status.ScheduledSize, requiredSize)
		status.RequiredSize = requiredSize
		status.ScheduledSize = updateSize
...
func (dsc *DiskScheduleController) reconcileResourceAllocation(logger *logrus.Entry, accScheduledSize, schedulableMaximum, scheduledSize, requireSize int64) (result longhorn.DiskScheduleResult, newScheduledSize, newAccScheduledSize int64) {
	...
	if newAccScheduledSize <= schedulableMaximum {
		logger.Info("Accept resource allocation")
		return longhorn.DiskScheduledResultScheduled, requireSize, newAccScheduledSize
	}
	logger.Info("Reject resource allocation")
	return longhorn.DiskScheduledResultRejected, scheduledSize, accScheduledSize
}
