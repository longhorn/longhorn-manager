...
func (dsc *DiskScheduleController) syncResourceAllocation(diskSchedule *longhorn.DiskSchedule) error {
	...
	overProvisioningPercentage, err := dsc.ds.GetSettingAsInt(types.SettingNameStorageOverProvisioningPercentage)
	if err != nil {
		dsc.logger.Errorf("failed to get setting %s for disk schedule %v, skip resource allocation", types.SettingNameStorageOverProvisioningPercentage, diskSchedule.Name)
		return nil
	}
	...
	for _, resource := range resourceList {
		...
		overProvisionLimit := scheduler.GetDiskOverProvisionLimit(diskStatus.StorageMaximum, diskSpec.StorageReserved, overProvisioningPercentage)
		result, updateSize, updateAccSize := dsc.reconcileResourceAllocation(resourceLogger, accScheduledSize, overProvisionLimit, status.Size, requiredSize)
...
func (dsc *DiskScheduleController) reconcileResourceAllocation(logger *logrus.Entry, accScheduledSize, overProvisionLimit, scheduledSize, requireSize int64) (result longhorn.DiskScheduleState, newScheduledSize, newAccScheduledSize int64) {
	logger = logger.WithFields(logrus.Fields{
		"accScheduledSize":   accScheduledSize,
		"overProvisionLimit": overProvisionLimit,
	...
	} else if newAccScheduledSize <= overProvisionLimit {
