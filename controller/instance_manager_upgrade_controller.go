...
func (imuc *InstanceManagerUpgradeController) syncInstanceManagerUpgrade(key string) (err error) {
	...
	imu = imu.DeepCopy()
	if imu.DeletionTimestamp == nil && !util.FinalizerExists(longhornFinalizerKey, imu) {
		if err := util.AddFinalizer(longhornFinalizerKey, imu); err != nil {
			return err
		}
		imu, err = imuc.ds.UpdateInstanceManagerUpgrade(imu)
		if err != nil {
			return err
		}
	}
...
func (imuc *InstanceManagerUpgradeController) reconcileRestoringEngines(imu *longhorn.InstanceManagerUpgrade, log *logrus.Entry) error {
	...
	if sourceNodeDeleted {
		...
		imuc.eventRecorder.Eventf(imu, corev1.EventTypeWarning, constant.EventReasonFailed,
			"Source node %v was deleted while restoring engines; abandoning restore", imu.Spec.NodeID)
		return nil
	}
