...
const (
	...
	instanceManagerUpgradeAbortReasonSourceNodeDeleted = "source-node-deleted"
...
func NewInstanceManagerUpgradeController(
	...
	imuc.cacheSyncs = append(imuc.cacheSyncs, ds.ReplicaInformer.HasSynced)
	imuc.cacheSyncs = append(imuc.cacheSyncs, ds.PodInformer.HasSynced)
	imuc.cacheSyncs = append(imuc.cacheSyncs, ds.NodeInformer.HasSynced)
	imuc.cacheSyncs = append(imuc.cacheSyncs, ds.SettingInformer.HasSynced)
...
func (imuc *InstanceManagerUpgradeController) isAuthorizedByUpgradeControl(imu *longhorn.InstanceManagerUpgrade) (bool, error) {
	control, err := imuc.ds.GetInstanceManagerUpgradeControlRO(types.InstanceManagerUpgradeControlName)
	if err != nil {
		if datastore.ErrorIsNotFound(err) || types.ErrorIsNotFound(err) {
			// A missing control may mean uninstall or a controller restart. Only
			// allow an IMU that already started to finish its recovery safely.
			return imu.Status.StartedAt != "" || imu.Status.AbortRequested, nil
		}
		return false, err
	}
...
func (imuc *InstanceManagerUpgradeController) syncInstanceManagerUpgrade(key string) (err error) {
	...
	if imu.DeletionTimestamp != nil {
		log.Warnf("Deleting instance manager upgrade %v", imu.Name)
		if imu.Status.State == longhorn.InstanceManagerUpgradeStateFailed {
			log.Info("Removing failed instance manager upgrade without engine restoration")
			return imuc.ds.RemoveFinalizerForInstanceManagerUpgrade(imu)
		}
		if types.IsActiveInstanceManagerUpgradeState(imu.Status.State) ||
			len(imu.Status.Engines) != 0 || len(imu.Status.PlannedDetachedReplicas) != 0 {
			if !imu.Status.AbortRequested {
				log.Infof("Instance manager upgrade %v is being deleted, restoring engines before cleanup", imu.Name)
				imu.Status.AbortRequested = true
			}
			if imu.Status.State != longhorn.InstanceManagerUpgradeStateRestoringEngines {
				imu.Status.State = longhorn.InstanceManagerUpgradeStateRestoringEngines
			}
			if err := imuc.reconcileStateMachine(imu, log); err != nil {
				return err
			}
	...
	if imu.Status.State != longhorn.InstanceManagerUpgradeStateCompleted &&
		imu.Status.State != longhorn.InstanceManagerUpgradeStateFailed &&
		!imu.Status.AbortRequested {
		authorized, authErr := imuc.isAuthorizedByUpgradeControl(imu)
		if authErr != nil {
			return authErr
		}
		if !authorized {
			log.Debugf("Waiting for IMU %v to be registered as the current upgrade by the IMUC", imu.Name)
...
func (imuc *InstanceManagerUpgradeController) reconcileStateMachine(imu *longhorn.InstanceManagerUpgrade, log *logrus.Entry) error {
	...
	case longhorn.InstanceManagerUpgradeStateRelocatingEngines:
		...
		// StartedAt is not reset between states, so one timeout applies to this IMU
		// attempt. A failed attempt is retried with a new IMU and timeout window.
		if imu.Status.AbortRequested {
			imuc.markAborted(imu, log,
	...
	case longhorn.InstanceManagerUpgradeStateWaitingForHealthyVolumes:
		if imu.Status.AbortRequested {
			imuc.markAborted(imu, log,
				"Abort requested (%s) while waiting for healthy volumes, marking upgrade as failed",
				longhorn.InstanceManagerUpgradeStateFailed)
			return nil
		}
...
func (imuc *InstanceManagerUpgradeController) enforceUpgradeTimeout(
	...
	if imu.Status.State == longhorn.InstanceManagerUpgradeStateWaitingForHealthyVolumes ||
		imu.Status.StartedAt == "" ||
		(imu.Status.State != longhorn.InstanceManagerUpgradeStatePending &&
			!types.IsActiveInstanceManagerUpgradeState(imu.Status.State)) {
		return false, nil
	}
	...
	timeoutMinutes, err := imuc.ds.GetSettingAsIntByDataEngine(types.SettingNameInstanceManagerUpgradeTimeout, longhorn.DataEngineTypeV2)
	if err != nil {
		log.WithError(err).Warnf("Failed to get %v setting, using default 60 minutes", types.SettingNameInstanceManagerUpgradeTimeout)
		timeoutMinutes = 60
	}
	...
	// StartedAt is not reset between states, so this IMU attempt has one timeout window
	// regardless of which state is stuck. A retry creates a new IMU with a new window.
...
func (imuc *InstanceManagerUpgradeController) reconcilePending(imu *longhorn.InstanceManagerUpgrade, log *logrus.Entry) error {
	...
	if nodeIM == nil {
		// Keep waiting in Pending while a source IM CR exists but its pod is
		// temporarily inactive; only a missing source IM enters the wait state.
		sourceIMs, err := imuc.ds.ListInstanceManagersByNodeRO(imu.Spec.NodeID, longhorn.InstanceManagerTypeAllInOne, longhorn.DataEngineTypeV2)
		if err != nil {
			return err
		}
		for _, sourceIM := range sourceIMs {
			if sourceIM.DeletionTimestamp == nil {
				log.Infof("Source IM %v exists on node %v but is temporarily inactive, keeping upgrade Pending", sourceIM.Name, imu.Spec.NodeID)
		...
		if converged {
			log.Infof("Node %v already running target image, checking volume recovery", imu.Spec.NodeID)
			imuc.markStartedAt(imu)
	...
	// The desired IM image can be ahead of the still-running pod after a failed patch.
	if nodeIM.Spec.Image == imu.Spec.TargetImage &&
		nodeIM.Status.CurrentState == longhorn.InstanceManagerStateRunning &&
		...
		if len(imu.Status.Engines) != 0 || len(imu.Status.PlannedDetachedReplicas) != 0 {
			// The source IM may have been upgraded after a prior reconcile persisted
			// the plan but failed to persist WaitingForSourceIM. Resume from the
			// persisted plan so engine restoration and volume-health checks are not
			// skipped.
			log.Infof("Node IM %v already runs target image with a persisted upgrade plan, resuming recovery", nodeIM.Name)
			imu.Status.State = longhorn.InstanceManagerUpgradeStateWaitingForSourceIM
			return nil
		}
		log.Infof("Node IM %v already running target image, checking volume recovery", nodeIM.Name)
		imuc.markStartedAt(imu)
		imu.Status.State = longhorn.InstanceManagerUpgradeStateWaitingForHealthyVolumes
		return imuc.reconcileWaitingForHealthyVolumes(imu, log)
	}
	...
	if err := imuc.validateNodeSupportsLiveUpgrade(imu.Spec.NodeID); err != nil {
	...
	drRecovered, err := imuc.areNodeDRVolumesRecovered(imu.Spec.NodeID, log)
	if err != nil {
		return err
	}
	if !drRecovered {
...
// ensureSourceIMUpgradeTriggered requests an in-place source IM image upgrade.
// Keep the existing Pod and its runtime so engine connections are preserved.
func (imuc *InstanceManagerUpgradeController) ensureSourceIMUpgradeTriggered(imu *longhorn.InstanceManagerUpgrade, log *logrus.Entry) error {
	...
	labels := types.GetInstanceManagerLabels(nodeIM.Spec.NodeID, imu.Spec.TargetImage, nodeIM.Spec.Type, nodeIM.Spec.DataEngine)
	if nodeIM.Labels == nil {
		nodeIM.Labels = map[string]string{}
	}
	for key, value := range labels {
		if key == types.GetLonghornLabelKey(types.LonghornLabelInstanceManagerImage) {
			nodeIM.Labels[key] = value
		}
	}

	pdb, err := imuc.ds.GetPDBRO(types.GetPDBName(nodeIM))
	...
	if pdb != nil {
		...
		if _, err := imuc.ds.UpdatePDB(pdb); err != nil {
			return errors.Wrapf(err, "failed to update PDB %v for in-place image upgrade", pdb.Name)
		}
	}
	...
	if err != nil {
		return errors.Wrapf(err, "failed to build label patch for instance manager pod %v", nodeIM.Name)
	}
	if _, err := imuc.ds.PatchPod(nodeIM.Name, podPatch); err != nil {
		return errors.Wrapf(err, "failed to patch instance manager pod %v labels", nodeIM.Name)
	}
...
func (imuc *InstanceManagerUpgradeController) reconcileRelocatingEngines(imu *longhorn.InstanceManagerUpgrade, log *logrus.Entry) error {
	...
	for volumeName, reloc := range imu.Status.Engines {
		...
		tempDiskSchedulable, err := imuc.hasSchedulableBlockDisk(reloc.TemporaryNodeID)
		if err != nil {
			return err
		}
		if !tempDiskSchedulable {
			log.Debugf("No schedulable block disk on temporary node %v, waiting before relocating volume %v", reloc.TemporaryNodeID, volumeName)
			continue
		}
...
func (imuc *InstanceManagerUpgradeController) isSourceIMReadyForEngineRestore(imu *longhorn.InstanceManagerUpgrade, im *longhorn.InstanceManager, pod *corev1.Pod, log *logrus.Entry) (bool, error) {
	...
	return imuc.isSourceNodeReadyForEngineRestore(im.Spec.NodeID)
}
...
func (imuc *InstanceManagerUpgradeController) reconcileRestoringEngines(imu *longhorn.InstanceManagerUpgrade, log *logrus.Entry) error {
	sourceNodeDeleted, err := imuc.ds.IsNodeDeleted(imu.Spec.NodeID)
	if err != nil {
		return errors.Wrapf(err, "failed to check whether source node %v was deleted", imu.Spec.NodeID)
	}
	if sourceNodeDeleted {
		imu.Status.AbortRequested = true
		imu.Status.AbortReason = instanceManagerUpgradeAbortReasonSourceNodeDeleted
		imu.Status.ErrorMsg = fmt.Sprintf("source node %v was deleted while waiting to restore engines", imu.Spec.NodeID)
		imu.Status.State = longhorn.InstanceManagerUpgradeStateFailed
		imu.Status.Engines = nil
		imu.Status.PlannedDetachedReplicas = nil
		log.Warnf("Source node %v was deleted while restoring engines; abandoning restore", imu.Spec.NodeID)
		return nil
	}
	...
	for volumeName, reloc := range imu.Status.Engines {
		...
		// Restoration is complete only after both the desired and observed engine
		// nodes return to the original node. A pending original-to-temporary
		// switchover can still report the original observed node while its desired
		// node points to the temporary node.
		if volume.Spec.EngineNodeID == reloc.OriginalNodeID &&
			volume.Status.CurrentEngineNodeID == reloc.OriginalNodeID {
			continue
		}
		sourceReady, err := imuc.isSourceIMReadyForRestore(imu.Spec.NodeID)
		if err != nil {
			return err
		}
		if !sourceReady {
			log.Debugf("Waiting for source IM on node %v to become ready before restoring volume %v", imu.Spec.NodeID, volumeName)
			allRestored = false
			continue
		}
	...
	if allRestored {
		...
		if imu.Status.AbortRequested {
			imuc.markAborted(imu, log,
				...
				longhorn.InstanceManagerUpgradeStateFailed)
			// The relocation plan is transient and no longer needed after abort cleanup.
			imu.Status.Engines = nil
			imu.Status.PlannedDetachedReplicas = nil
		} else {
...
func (imuc *InstanceManagerUpgradeController) isSourceIMReadyForRestore(nodeID string) (bool, error) {
	nodeIM, err := imuc.ds.GetNodeV2InstanceManagerRO(nodeID)
	if err != nil {
		if datastore.ErrorIsNotFound(err) || types.ErrorIsNotFound(err) {
			return false, nil
		}
	...
	if pod == nil || pod.DeletionTimestamp != nil || getInstanceManagerPodImage(pod, "instance-manager") != nodeIM.Spec.Image {
		return false, nil
	}
	for _, status := range pod.Status.ContainerStatuses {
		if status.Name == "instance-manager" {
			if !status.Ready || status.State.Running == nil {
				return false, nil
			}
			return imuc.isSourceNodeReadyForEngineRestore(nodeID)
		}
	}
	return false, nil
}

// isSourceNodeReadyForEngineRestore requires a schedulable block disk only
// when the source node has block disks configured. A source node without block
// disks can still host an engine that uses a backend on another node.
func (imuc *InstanceManagerUpgradeController) isSourceNodeReadyForEngineRestore(nodeID string) (bool, error) {
	node, err := imuc.ds.GetNodeRO(nodeID)
	if err != nil {
		return false, err
	}
	...
	return !hasBlockDisk, nil
}

// A migration target needs a schedulable block disk before the engine is moved.
func (imuc *InstanceManagerUpgradeController) hasSchedulableBlockDisk(nodeID string) (bool, error) {
	node, err := imuc.ds.GetNodeRO(nodeID)
	if err != nil {
		return false, err
	}
...
func (imuc *InstanceManagerUpgradeController) reconcileWaitingForHealthyVolumes(imu *longhorn.InstanceManagerUpgrade, log *logrus.Entry) error {
	...
	for volumeName, reloc := range imu.Status.Engines {
		...
		if volume.Status.State == longhorn.VolumeStateDetached {
			log.Infof("Volume %v detached while waiting for health, removing from plan", volumeName)
			delete(imu.Status.Engines, volumeName)
			delete(imu.Status.PlannedDetachedReplicas, volumeName)
			continue
		}
	...
	for volumeName := range imu.Status.PlannedDetachedReplicas {
		...
		if volume.Status.State == longhorn.VolumeStateDetached {
			log.Infof("Volume %v detached while waiting for planned replica health, removing from plan", volumeName)
			delete(imu.Status.PlannedDetachedReplicas, volumeName)
			continue
		}
	...
	if !allHealthy || !drRecovered {
		return nil
	}

	log.Infof("All restored volumes are healthy on original nodes or detached, upgrade completed")
	imu.Status.State = longhorn.InstanceManagerUpgradeStateCompleted

	return nil
}

// areNodeDRVolumesRecovered checks the current topology rather than the relocation
// plan, which intentionally excludes DR volumes. Failed/stopped replicas still
// identify affected volumes after their engines have detached.
func (imuc *InstanceManagerUpgradeController) areNodeDRVolumesRecovered(nodeID string, log *logrus.Entry) (bool, error) {
	volumes, err := imuc.ds.ListVolumesRO()
	if err != nil {
		return false, err
	}
	for _, volume := range volumes {
		if volume.DeletionTimestamp != nil || !types.IsDataEngineV2(volume.Spec.DataEngine) ||
			!volume.Status.IsStandby {
			continue
		}
		...
		engines, err := imuc.ds.ListVolumeEnginesRO(volume.Name)
		if err != nil {
			return false, err
		}
		affected := volume.Status.CurrentEngineNodeID == nodeID || volume.Status.CurrentNodeID == nodeID
		for _, engine := range engines {
			affected = affected || engine.Spec.NodeID == nodeID
		}
		for _, replica := range replicas {
			affected = affected || replica.Spec.NodeID == nodeID
		}
		if !affected {
			continue
		}
		if volume.Status.State != longhorn.VolumeStateAttached || volume.Status.Robustness != longhorn.VolumeRobustnessHealthy {
			log.Debugf("Waiting for DR volume %v recovery on node %v: state=%v robustness=%v", volume.Name, nodeID, volume.Status.State, volume.Status.Robustness)
			return false, nil
		}
		engine, err := imuc.ds.PickVolumeCurrentEngine(volume, engines)
		if err != nil {
			return false, err
		}
		if !isDRVolumeEngineRecovered(volume, engine, replicas) {
			log.Debugf("Waiting for DR volume %v engine and replicas to recover on node %v", volume.Name, nodeID)
			return false, nil
		}
	}
	return true, nil
...
func isDRVolumeEngineRecovered(volume *longhorn.Volume, engine *longhorn.Engine, replicas map[string]*longhorn.Replica) bool {
	if engine == nil || engine.Status.CurrentState != longhorn.InstanceStateRunning ||
		len(engine.Status.ReplicaModeMap) != volume.Spec.NumberOfReplicas {
		return false
	}
	// Rebuilding completes when all replicas are RW. Backup restore progress is
	// independent and must not hold up an otherwise healthy node upgrade.
	for name, mode := range engine.Status.ReplicaModeMap {
		replica := replicas[name]
		if mode != longhorn.ReplicaModeRW || replica == nil ||
			replica.Status.CurrentState != longhorn.InstanceStateRunning {
			return false
		}
		address := engine.Status.CurrentReplicaAddressMap[name]
		// Do not accept backend addresses left over from before the IM restart.
		if address != imutil.GetURL(replica.Status.StorageIP, replica.Status.Port) {
			return false
		}
	}
	return true
...
func (imuc *InstanceManagerUpgradeController) buildEngineRelocationPlan(imu *longhorn.InstanceManagerUpgrade) (map[string]longhorn.EngineRelocation, error) {
	...
	for _, engine := range engines {
		...
		if volume.Spec.EngineNodeID != "" && volume.Status.CurrentEngineNodeID != "" &&
			volume.Spec.EngineNodeID != volume.Status.CurrentEngineNodeID {
			return nil, fmt.Errorf("%w: volume %v engine switchover is still in progress",
				errUpgradePrecondition, volume.Name)
		}

		currentEngine, err := imuc.ds.GetVolumeCurrentEngine(volume.Name)
		if err != nil {
			if datastore.ErrorIsNotFound(err) || types.ErrorIsNotFound(err) {
				continue
			}
		...
		if currentEngine == nil || currentEngine.Name != engine.Name || currentEngine.Spec.NodeID != imu.Spec.NodeID {
			continue
		}
...
func (imuc *InstanceManagerUpgradeController) validateNodeSupportsLiveUpgrade(nodeID string) error {
	shards, err := imuc.ds.ListShardsByNode(nodeID)
	if err != nil {
		return errors.Wrapf(err, "failed to list shards on node %v", nodeID)
	}
	shardedVolumesOnNode := map[string]struct{}{}
	for _, shard := range shards {
		if volumeName := shard.Labels[types.LonghornLabelVolume]; volumeName != "" {
			shardedVolumesOnNode[volumeName] = struct{}{}
		}
	}
	...
	for _, volume := range volumes {
		if volume.DeletionTimestamp != nil || volume.Status.State != longhorn.VolumeStateAttached {
			continue
		}
		if types.IsDataEngineV2(volume.Spec.DataEngine) && volume.Status.IsStandby {
			replicas, err := imuc.ds.ListVolumeReplicasRO(volume.Name)
			if err != nil {
				return errors.Wrapf(err, "failed to list replicas for DR volume %v", volume.Name)
			}
			affected := volume.Status.CurrentNodeID == nodeID || volume.Status.CurrentEngineNodeID == nodeID
			hasOtherReplica := false
			for _, replica := range replicas {
				if replica.DeletionTimestamp != nil || replica.Spec.NodeID == "" {
					continue
				}
				if replica.Spec.NodeID == nodeID {
					affected = true
				} else {
					// Health is checked separately so a temporarily failed remote
					// replica blocks on recovery rather than unsupported topology.
					hasOtherReplica = true
				}
			}
			if affected && !hasOtherReplica {
				return fmt.Errorf("%w: DR volume %v has no replica outside upgrade node %v; single-replica or co-located DR volumes are not supported for live upgrade",
					errUpgradeUnsupported, volume.Name, nodeID)
			}
		}
		_, hasShardOnNode := shardedVolumesOnNode[volume.Name]
		if volume.Status.CurrentNodeID != nodeID && volume.Status.CurrentEngineNodeID != nodeID && !hasShardOnNode {
			continue
		}
		// Sharded volumes use a ShardGroup backend rather than Replica CRs. The
		// live-upgrade relocation flow requires a healthy RW replica on the
		// temporary node, so it cannot safely switch a sharded volume.
		if volume.Spec.DataLayout.Type == longhorn.VolumeDataLayoutTypeSharded {
			return fmt.Errorf("%w: node %v has sharded volume %v, which is not supported for live upgrade",
				errUpgradeUnsupported, nodeID, volume.Name)
		}
		if volume.Spec.Frontend == longhorn.VolumeFrontendUblk {
			return fmt.Errorf("%w: node %v has ublk frontend volume %v, which is not supported for live upgrade",
				errUpgradeUnsupported, nodeID, volume.Name)
		}
	}
...
// detachPlannedReplicasFromCurrentEngines removes source-node replicas before
// the IM upgrade, preventing the old IM from using replicas on the new IM.
func (imuc *InstanceManagerUpgradeController) detachPlannedReplicasFromCurrentEngines(imu *longhorn.InstanceManagerUpgrade, log *logrus.Entry) (bool, error) {
	// Validate the attached volume and running current engine, track the
	// asynchronous detach state, and keep a non-planned RW replica available.
	// Return true only after all planned replicas are detached and converged;
	// return false to keep waiting, or an error when the detach cannot proceed.
	for volumeName, plannedReplicas := range imu.Status.PlannedDetachedReplicas {
		volume, err := imuc.ds.GetVolumeRO(volumeName)
		if err != nil {
			if datastore.ErrorIsNotFound(err) || types.ErrorIsNotFound(err) {
		...
		for plannedReplicaIndex, plannedReplica := range plannedReplicas {
			replicaName := plannedReplica.Name
			latestEngine, err := imuc.ds.GetVolumeCurrentEngine(volumeName)
			if err != nil {
				if datastore.ErrorIsNotFound(err) || types.ErrorIsNotFound(err) {
					return false, nil
				}
				return false, errors.Wrapf(err, "failed to refresh current engine for volume %v before planned replica detach", volumeName)
			}
			if latestEngine == nil || latestEngine.Name != engine.Name ||
				latestEngine.Status.CurrentState != longhorn.InstanceStateRunning {
				return false, nil
			}
			engine = latestEngine
			replicaAddress := strings.TrimPrefix(plannedReplica.Address, plannedDetachedReplicaAddressDetachedPrefix)
			// A detached marker means a previous remove was already requested.
			detachAttempted := replicaAddress != plannedReplica.Address

			// Engine state is eventually consistent across these maps. Keep
			// reconciling while the replica exists in any pending state:
			// - inSpec is the desired replica list,
			// - inCurrent is the runtime connection
			// - replicaModeTracked means the engine still tracks the replica's mode.
			_, inSpec := engine.Spec.ReplicaAddressMap[replicaName]
			_, inCurrent := engine.Status.CurrentReplicaAddressMap[replicaName]
			replicaModeTracked := false
			for _, key := range getPlannedDetachedReplicaStatusKeys(plannedReplica) {
				if _, ok := engine.Status.ReplicaModeMap[key]; ok {
					replicaModeTracked = true
					break
				}
			}

			// Verify a surviving RW replica before removing a replica that is
			// still present in the engine or has not been detached yet.
			if inSpec || inCurrent || replicaModeTracked || (replicaAddress != "" && !detachAttempted) {
				replicas, err := imuc.ds.ListVolumeReplicasRO(volumeName)
				if err != nil {
					return false, errors.Wrapf(err, "failed to refresh replicas for volume %v before planned replica detach", volumeName)
				}
				plannedReplicaNames := make(map[string]struct{}, len(plannedReplicas))
				for _, planned := range plannedReplicas {
					plannedReplicaNames[planned.Name] = struct{}{}
				}
				hasNonPlannedRWReplica := false
				for name, replica := range replicas {
					if _, planned := plannedReplicaNames[name]; planned || replica.Spec.EngineName != engine.Name ||
						replica.Spec.FailedAt != "" || replica.DeletionTimestamp != nil {
						continue
					}
					if engine.Status.ReplicaModeMap[name] == longhorn.ReplicaModeRW {
						hasNonPlannedRWReplica = true
						break
					}
				}
				if !hasNonPlannedRWReplica {
					log.Debugf("Waiting to detach planned replica %v from volume %v because no non-planned RW replica is currently available", replicaName, volumeName)
					return false, nil
				}
			}
			// The engine no longer tracks this replica, so the detach is complete.
			if !inSpec && !inCurrent && !replicaModeTracked {
				if replicaAddress == "" || detachAttempted {
					continue
				}
			}

			// Remove the replica while it is still connected, tracked by mode, or
			// has not yet received a detach request.
			if inCurrent || replicaModeTracked || (replicaAddress != "" && !detachAttempted) {
				...
				log.Infof("Detached planned replica %v from current engine %v before upgrading source IM", replicaName, engine.Name)
			}
...
func (imuc *InstanceManagerUpgradeController) handleTemporaryNodeFailureForVolume(
	...
	if err != nil || newTempNode == reloc.TemporaryNodeID {
		// Do not move the engine to the original node until its IM is running.
		sourceReady, sourceErr := imuc.isSourceIMReadyForRestore(imu.Spec.NodeID)
		if sourceErr != nil {
			return false, "", sourceErr
		}
		if !sourceReady {
			log.Debugf("Keeping volume %v on temporary node %v because source IM on node %v is not ready", volumeName, reloc.TemporaryNodeID, imu.Spec.NodeID)
			return false, "", nil
		}
	...
	updatedReloc := reloc
	newTempDiskSchedulable, err := imuc.hasSchedulableBlockDisk(newTempNode)
	if err != nil {
		return false, "", err
	}
	if !newTempDiskSchedulable {
		log.Debugf("No schedulable block disk on replacement temporary node %v for volume %v, waiting", newTempNode, volumeName)
		return false, "", nil
	}
...
func (imuc *InstanceManagerUpgradeController) selectTemporaryNode(sourceNodeID, volumeName string, currentPlan map[string]longhorn.EngineRelocation) (string, error) {
	...
	for _, r := range replicas {
		...
		if r.Spec.FailedAt != "" || r.Spec.HealthyAt == "" || !r.Spec.Active || r.Spec.EngineName == "" {
			continue
		}
		engine, exists := engines[r.Spec.EngineName]
		if !exists {
			engine, err = imuc.ds.GetEngineRO(r.Spec.EngineName)
			if err != nil {
				if datastore.ErrorIsNotFound(err) || types.ErrorIsNotFound(err) {
					continue
				}
				return "", errors.Wrapf(err, "failed to get engine %v for replica %v", r.Spec.EngineName, r.Name)
			}
			engines[r.Spec.EngineName] = engine
		}
		if engine.Status.ReplicaModeMap[r.Name] != longhorn.ReplicaModeRW {
			continue
		}
	...
	for nodeID := range healthyReplicaNodes {
		...
		if im != nil {
			hasSchedulableDisk, err := imuc.hasSchedulableBlockDisk(nodeID)
			if err != nil {
				return "", errors.Wrapf(err, "failed to check schedulable block disk on candidate temporary node %v for volume %v", nodeID, volumeName)
			}
			if !hasSchedulableDisk {
				continue
			}
