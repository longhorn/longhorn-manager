...
// phase without creating member snapshots.
func (s *SnapshotGroupControllerSuite) TestRestoreGuardRestoresTerminalPhaseWithoutTakingSnapshots(c *C) {
	for _, phase := range []longhorn.SnapshotGroupPhase{longhorn.SnapshotGroupPhaseReady, longhorn.SnapshotGroupPhaseFailed} {
		...
		c.Assert(updated.Status.ReadyToUse, Equals, phase == longhorn.SnapshotGroupPhaseReady)
...
// TestDegradedAfterMemberLossKeepsReadyToUse verifies that losing a member
// snapshot after Ready marks the member lost and raises the Degraded
// condition, while the group stays Ready and ReadyToUse.
func (s *SnapshotGroupControllerSuite) TestDegradedAfterMemberLossKeepsReadyToUse(c *C) {
	f := newSnapshotGroupControllerFixture(c)
	group := newTestSnapshotGroup("vol-1", "vol-2")
	annotateTerminalPhase(group, longhorn.SnapshotGroupPhaseReady)
...
// lingers, raising the Degraded condition.
