...
// newRestoreErrorTestCase builds a standby volume whose engine reports the same restore
// error for every replica, which is how a v2 engine-scoped restore failure surfaces.
func newRestoreErrorTestCase(dataEngine longhorn.DataEngineType) (*longhorn.Volume, map[string]*longhorn.Engine, map[string]*longhorn.Replica) {
	v := newVolume(TestVolumeName, 2)
	v.Spec.DataEngine = dataEngine
	v.Spec.FromBackup = backupstore.EncodeBackupURL(TestBackupName, TestVolumeName, TestBackupTarget)
	v.Spec.Standby = true
	v.Status.OwnerID = TestNode1
	...
	for _, node := range []string{TestNode1, TestNode2} {
		r := newReplicaForVolume(v, e, node, TestDiskID1)
		r.Spec.DataEngine = dataEngine
		r.Spec.HealthyAt = getTestNow()
		r.Spec.LastHealthyAt = r.Spec.HealthyAt
		r.Spec.DesireState = longhorn.InstanceStateRunning
		r.Status.CurrentState = longhorn.InstanceStateRunning
		...
		replicaAddress := imutil.GetURL(r.Status.StorageIP, r.Status.Port)
		e.Spec.ReplicaAddressMap[r.Name] = replicaAddress
		e.Status.ReplicaModeMap[r.Name] = longhorn.ReplicaModeRW
		e.Status.ReplicaTransitionTimeMap[r.Name] = getTestNow()
		e.Status.RestoreStatus[replicaAddress] = &longhorn.RestoreStatus{
			Error: "failed to sync NVMe device before close: input/output error",
		}
		rs[r.Name] = r
	}

	return v, map[string]*longhorn.Engine{e.Name: e}, rs
}

func (s *TestSuite) TestReconcileEngineReplicaStateV2KeepsReplicasOnRestoreError(c *C) {
	datastore.SkipListerCheck = true
	...
	v, es, rs := newRestoreErrorTestCase(longhorn.DataEngineTypeV2)

	c.Assert(vc.ReconcileEngineReplicaState(v, es, rs), IsNil)

	// A DR volume cannot be auto-salvaged, so failing every replica on an engine-scoped
	// restore error would leave it unrecoverable.
	for _, r := range rs {
...
func (s *TestSuite) TestReconcileEngineReplicaStateV1FailsReplicasOnRestoreError(c *C) {
	datastore.SkipListerCheck = true
	...
	v, es, rs := newRestoreErrorTestCase(longhorn.DataEngineTypeV1)

	c.Assert(vc.ReconcileEngineReplicaState(v, es, rs), IsNil)

	// v1 restores run per replica, so the error really does belong to the replica.
	for _, r := range rs {
		c.Assert(r.Spec.FailedAt, Not(Equals), "")
		c.Assert(r.Spec.DesireState, Equals, longhorn.InstanceStateStopped)
	}
}
...
func (s *TestSuite) runTestCases(c *C, testCases map[string]*VolumeTestCase) {
	...
	for name, tc := range testCases {
		...
		for _, instanceManager := range []*longhorn.InstanceManager{instanceManager1, instanceManager2} {
			instanceManager.Status.IPFamily = ""
