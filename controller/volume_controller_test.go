...
func (s *TestSuite) TestTooManySnapshotsThresholdBehavior(c *C) {
	...
	tests := map[string]struct {
		snapshotCount        int
		snapshotSizes        []int64
		invalidSnapshotSizes map[string]string
		specThreshold        int
		...
		"snapshot data corruption": {
			snapshotCount:        3,
			invalidSnapshotSizes: map[string]string{"s0": "invalid_size", "s1": "20", "s2": "30"},
			specThreshold:        10,
			specSizeThreshold:    100,
			expectStatus:         longhorn.ConditionStatusTrue,
			expectReason:         longhorn.VolumeConditionReasonSnapshotDataCorruption,
			expectMessage:        `Snapshot data corruption detected: failed to parse snapshot s0 size invalid_size: strconv.ParseInt: parsing "invalid_size": invalid syntax`,
		},
	}
	...
	for name, tt := range tests {
		...
		for _, eng := range tcase.engines {
			if tt.invalidSnapshotSizes != nil {
				// Construct snapshots map with invalid sizes for corruption testing
				eng.Status.Snapshots = map[string]*longhorn.SnapshotInfo{
					"volume-head": {Name: "volume-head", Size: "0"},
				}
				for name, size := range tt.invalidSnapshotSizes {
					eng.Status.Snapshots[name] = &longhorn.SnapshotInfo{
						Name: name,
						Size: size,
					}
					// reconcileVolumeSize sums parseable sizes and skips invalid ones
					if parsed, err := util.ConvertSize(size); err == nil {
						expectedActualSize += parsed
					}
				}
			} else {
