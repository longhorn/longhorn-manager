...
func (vbc *VolumeRebuildingController) enqueueVolumeNodeReady(old, cur interface{}) {
	...
	if curNode.Status.Conditions == nil {
		return
	}
	...
	readyNotChangedOrFalse := oldReadyStatus == curReadyStatus || curReadyStatus == longhorn.ConditionStatusFalse
	schedulableNotChangedOrFalse := oldSchedulableStatus == curSchedulableStatus || curSchedulableStatus == longhorn.ConditionStatusFalse
	schedulingNotChangedOrFalse := oldNode.Spec.AllowScheduling == curNode.Spec.AllowScheduling || !curNode.Spec.AllowScheduling

	if readyNotChangedOrFalse && schedulableNotChangedOrFalse && schedulingNotChangedOrFalse {
		return
	}
