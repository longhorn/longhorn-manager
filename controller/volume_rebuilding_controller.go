...
func (vbc *VolumeRebuildingController) enqueueSetting(obj interface{}) {
	...
	for _, v := range vs {
		if v.Spec.OfflineRebuilding != longhorn.VolumeOfflineRebuildingIgnored {
			continue
		}
		vbc.enqueueVolume(v)
	}
...
func (vbc *VolumeRebuildingController) reconcile(volName string) (err error) {
	...
	if isOfflineRebuildEnabled && !util.IsHigherPriorityVATicketExisting(va, longhorn.AttacherTypeVolumeRebuildingController) {
		if vol.Status.State == longhorn.VolumeStateDetached {
			va, err = vbc.syncLHVolumeAttachmentForOfflineRebuild(vol, va, rebuildingAttachmentTicketID)
			if err != nil {
				return err
			}
			return nil
		...
		isVolumeInRebuilding, err := vbc.isVolumeReplicasRebuilding(vol, engine)
		if err != nil {
			return err
		}
...
func (vbc *VolumeRebuildingController) isVolumeOfflineRebuildEnabled(offlineRebuilding longhorn.VolumeOfflineRebuilding) (bool, error) {
	...
	return offlineReplicaRebuildingValue && offlineRebuilding != longhorn.VolumeOfflineRebuildingDisabled, nil
}
...
func (vbc *VolumeRebuildingController) syncLHVolumeAttachmentForOfflineRebuild(vol *longhorn.Volume, va *longhorn.VolumeAttachment, attachmentID string) (*longhorn.VolumeAttachment, error) {
	...
	areAllReplicasHealthy, err := vbc.isVolumeReplicasHealthy(vol.Spec.NumberOfReplicas, replicas)
	if err != nil {
		return va, err
	}
	if !areAllReplicasHealthy {
...
func (vbc *VolumeRebuildingController) isVolumeReplicasHealthy(numberOfReplicas int, replicas map[string]*longhorn.Replica) (bool, error) {
	...
	return healthyCount >= numberOfReplicas, nil
}
...
func (vbc *VolumeRebuildingController) isVolumeReplicasRebuilding(vol *longhorn.Volume, engine *longhorn.Engine) (bool, error) {
	healthyCount := 0
	...
	return healthyCount < vol.Spec.NumberOfReplicas, nil
}
