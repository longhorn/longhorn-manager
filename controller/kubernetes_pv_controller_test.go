...
func newTestKubernetesPVController(lhClient *lhfake.Clientset, kubeClient *fake.Clientset, extensionsClient *apiextensionsfake.Clientset, informerFactories *util.InformerFactories) (*KubernetesPVController, error) {
	...
	kc, err := NewKubernetesPVController(logger, ds, scheme.Scheme, kubeClient)
	if err != nil {
		return nil, err
	}
...
func (s *TestSuite) TestSyncKubernetesStatusIgnoresVolumeOwnerID(c *C) {
	...
	tc := generateKubernetesTestCaseTemplate()
	// The volume is owned by an arbitrary node. With the old guard this
	// reconcile would early-return and leave KubernetesStatus empty; in
	// single-mode it must still be populated.
...
func (s *TestSuite) TestCleanupForPVDeletionIgnoresVolumeOwnerID(c *C) {
	...
	volume := newVolume(TestVolumeName, 2)
	volume.Status.OwnerID = TestNode2 // owned by an arbitrary node
