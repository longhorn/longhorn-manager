...
func engineBinaryExists(image string, ds *datastore.DataStore) (bool, error) {
	...
	engineBinaryPath := filepath.Join(controlPath, types.EngineBinaryDirectorySubpath, types.GetImageCanonicalName(image), types.EngineBinaryName)
					...
					Volumes: []corev1.Volume{
						{
							...
							VolumeSource: corev1.VolumeSource{
								HostPath: &corev1.HostPathVolumeSource{
									Path: filepath.Join(controlPath, types.EngineBinaryDirectorySubpath, types.GetImageCanonicalName(image)),
								},
							},
						},
					},
