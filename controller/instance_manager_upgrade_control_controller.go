...
const (
	...
	// imuCreateMaxAttempts bounds retries when a generated IMU name already exists.
	imuCreateMaxAttempts = 3
)
...
func (c *InstanceManagerUpgradeControlController) isResponsibleForSetting(obj interface{}) bool {
	...
	return types.SettingName(setting.Name) == types.SettingNameAllowInstanceManagerAutomaticUpgrade ||
...
func (c *InstanceManagerUpgradeControlController) reconcile(imuc *longhorn.InstanceManagerUpgradeControl) (bool, error) {
	log := c.logger.WithField("instanceManagerUpgradeControl", imuc.Name)
	...
	repairedCurrentNode, err := c.repairIMUCInvariants(imuc)
	if err != nil {
		return false, err
	}
	...
	if imuc.Status.CurrentNode != "" {
		var err error
		active, err = c.processCurrentNode(imuc)
		if err != nil {
			return false, err
		}
	}
	...
	recoveringOrphan, err := c.recoverOrphanedNodes(imuc)
	if err != nil {
		return false, err
	}
	if recoveringOrphan {
	...
	if err := c.handleTargetImageChange(imuc); err != nil {
		return false, err
	}
	...
	allowed, err := c.ds.GetSettingAsBoolByDataEngine(types.SettingNameAllowInstanceManagerAutomaticUpgrade, longhorn.DataEngineTypeV2)
	if err != nil {
		return false, err
	}
	if !allowed {
		log.Debugf("Skipping next V2 instance manager upgrade because %v is disabled", types.SettingNameAllowInstanceManagerAutomaticUpgrade)
		return false, nil
	}
	...
	if nextNode == "" {
		return c.retryFailedNode(imuc)
	}

	if err := c.startNodeUpgrade(imuc, nextNode); err != nil {
		return false, err
	}
	return true, nil
}
...
func (c *InstanceManagerUpgradeControlController) retryFailedNode(imuc *longhorn.InstanceManagerUpgradeControl) (bool, error) {
	log := c.logger.WithField("instanceManagerUpgradeControl", imuc.Name)
...
// repairIMUCInvariants restores the bookkeeping that the one-node-at-a-time rolling upgrade
// depends on: Status.CurrentNode must name an entry that is in-progress, and an in-progress entry
// must carry an IMU name to remain trackable. In-progress entries other than CurrentNode are left
// intact so recoverOrphanedNodes can fail their IMU as well. Returns true when anything was
// repaired, so the caller ends the round instead of scheduling from state it just rewrote.
func (c *InstanceManagerUpgradeControlController) repairIMUCInvariants(imuc *longhorn.InstanceManagerUpgradeControl) (bool, error) {
	log := c.logger.WithField("instanceManagerUpgradeControl", imuc.Name)
...
func (c *InstanceManagerUpgradeControlController) processCurrentNode(imuc *longhorn.InstanceManagerUpgradeControl) (active bool, err error) {
	log := c.logger.WithField("instanceManagerUpgradeControl", imuc.Name)

	nodeID := imuc.Status.CurrentNode
	nodeInfo, exists := imuc.Status.Nodes[nodeID]
	if !exists {
		return false, fmt.Errorf("BUG: current node %v is missing from instance manager upgrade status", nodeID)
	}
...
func (c *InstanceManagerUpgradeControlController) recoverOrphanedNodes(imuc *longhorn.InstanceManagerUpgradeControl) (bool, error) {
	log := c.logger.WithField("instanceManagerUpgradeControl", imuc.Name)
	...
	for nodeID, nodeInfo := range imuc.Status.Nodes {
		...
		log.Warnf("Marked orphaned IMU %v for node %v as failed", imu.Name, nodeID)
	}
	return false, nil
}
...
func (c *InstanceManagerUpgradeControlController) handleTargetImageChange(imuc *longhorn.InstanceManagerUpgradeControl) error {
	log := c.logger.WithField("instanceManagerUpgradeControl", imuc.Name)
...
func (c *InstanceManagerUpgradeControlController) ensureIMUForNode(targetImage, nodeID string) (string, error) {
	...
	for range imuCreateMaxAttempts {
...
func (c *InstanceManagerUpgradeControlController) pickNextPendingNode(imuc *longhorn.InstanceManagerUpgradeControl) string {
	...
	ims, err := c.ds.ListInstanceManagersBySelectorRO("", "", longhorn.InstanceManagerTypeAllInOne, longhorn.DataEngineTypeV2)
	if err != nil {
		c.logger.WithError(err).Warn("Failed to list V2 instance managers while picking next node")
	} else {
		for _, im := range ims {
