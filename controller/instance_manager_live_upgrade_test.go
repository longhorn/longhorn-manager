...
func (s *TestSuite) TestProcessCurrentNodeSkipsRetryWhenSourceNodeIsDeleted(c *C) {
	...
	active, err := control.processCurrentNode(imuc, logrus.NewEntry(logrus.StandardLogger()))
	c.Assert(err, IsNil)
...
func (s *TestSuite) TestRetryFailedNodeResumesPersistedRelocationPlan(c *C) {
	...
	active, err := control.retryFailedNode(imuc, logrus.NewEntry(logrus.StandardLogger()))
	c.Assert(err, IsNil)
...
func (s *TestSuite) TestInstanceManagerUpgradeControlRespectsScheduledNewTargetCycle(c *C) {
	...
	setting := newSetting(string(types.SettingNameAllowV2InstanceManagerAutomaticUpgrade), "true")
