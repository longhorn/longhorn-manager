...
func (bic *BackingImageController) syncBackingImageFileInfo(bi *longhorn.BackingImage) (err error) {
	...
	for _, diskSchedule := range diskScheduleMap {
		allocationStatus, exist := diskSchedule.Status.ScheduledBackingImageStatus[bi.Name]
		if !exist {
			continue
		}
		if allocationStatus.RequiredSize != bi.Status.Size {
			continue
		}
		if allocationStatus.Result == longhorn.DiskScheduledResultRejected {
			message := "disk allocation is rejected"
			if err := bic.updateStatusWithFileInfo(bi, diskSchedule.Name, message, bi.Status.Checksum, longhorn.BackingImageStateFailed, 0, longhorn.DataEngineTypeV1); err != nil {
				return err
			}
		}
	}
...
func (bic *BackingImageController) syncDiskSchedule(bi *longhorn.BackingImage) error {
	...
	for diskUUID, diskSchedule := range diskSchedules {
		...
		if _, isDiskSchedulingCopy := diskSchedule.Spec.SchedulingBackingImages[bi.Name]; !isDiskSchedulingCopy {
			continue
		}
		// backing image is not scheduling onto the disk, but disk schedule has the scheduling request
