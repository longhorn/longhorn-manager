...
func (bvc *BackupVolumeController) reconcile(backupVolumeName string) (err error) {
	...
	sourceBackupVolumeName := backupVolume.Spec.VolumeName
	...
	if !backupVolume.DeletionTimestamp.IsZero() {
		...
		if needsCleanupRemoteData {
			...
			if err := backupTargetClient.BackupVolumeDelete(backupTargetClient.URL, sourceBackupVolumeName, backupTargetClient.Credential); err != nil {
	...
	res, err := backupTargetClient.BackupNameList(backupTargetClient.URL, sourceBackupVolumeName, backupTargetClient.Credential)
	if err != nil {
	...
	clusterBackups, err := bvc.ds.ListBackupsWithBackupTargetAndBackupVolumeRO(backupTargetName, sourceBackupVolumeName)
	if err != nil {
	...
	for backupName := range backupsToPull {
		...
		backupURL := backupstore.EncodeBackupURL(backupName, sourceBackupVolumeName, backupTargetClient.URL)
		...
		if _, err = bvc.ds.CreateBackup(backup, sourceBackupVolumeName); err != nil && !apierrors.IsAlreadyExists(err) {
	...
	backupVolumeMetadataURL := backupstore.EncodeBackupURL("", sourceBackupVolumeName, backupTargetClient.URL)
