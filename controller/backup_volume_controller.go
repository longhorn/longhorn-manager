...
func (bvc *BackupVolumeController) reconcile(backupVolumeName string) (err error) {
	...
	for backupName := range backupsToPull {
		...
		// If the block size is unavailable from remote backup, it still creates a backup with invalid block size,
		// but the volume restoring will be rejected by the volume validator.
