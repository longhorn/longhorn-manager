...
func NewReplicaController(
	...
	if _, err = ds.DiskScheduleInformer.AddEventHandlerWithResyncPeriod(cache.ResourceEventHandlerFuncs{
		UpdateFunc: rc.enqueueDiskScheduleChange,
	}, 0); err != nil {
		return nil, err
	}
...
func (rc *ReplicaController) syncReplicaDisk(replica *longhorn.Replica) (allocated bool, err error) {
	...
	if lastAllocationResult.Result != longhorn.DiskScheduledResultScheduled {
		...
		return false, fmt.Errorf("disk schedule %v rejected replica allocation", replica.Spec.DiskID)
	}
...
func (rc *ReplicaController) CreateInstance(obj interface{}) (*longhorn.InstanceProcess, error) {
	...
	if schedulingErr != nil {
		// disk allocation is failed or rejected
		return nil, schedulingErr
	}
	if !isScheduledOnDisk {
		// waiting for disk schedule to reconcile allocation
		return nil, nil
	}
...
func (rc *ReplicaController) enqueueDiskScheduleChange(oldObj, curObj interface{}) {
	oldDs, err := eventObjToTypedObj[longhorn.DiskSchedule](oldObj)
	if err != nil {
		utilruntime.HandleError(err)
		return
	}

	curDs, err := eventObjToTypedObj[longhorn.DiskSchedule](curObj)
	if err != nil {
		utilruntime.HandleError(err)
		return
	}
