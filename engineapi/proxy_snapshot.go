...
func (p *Proxy) SnapshotClone(e *longhorn.Engine, snapshotName, fromEngineAddress, fromVolumeName, fromEngineName string,
	fileSyncHTTPClientTimeout, grpcTimeoutSeconds int64,
	srcReplicaName, srcReplicaAddress, dstReplicaName, dstReplicaAddress, cloneMode string) (err error) {
	return p.grpcClient.SnapshotClone(string(e.Spec.DataEngine), e.Name, e.Spec.VolumeName, p.DirectToURL(e),
		...
		srcReplicaName, srcReplicaAddress, dstReplicaName, dstReplicaAddress, cloneMode)
}
