...
func (p *Proxy) SnapshotBackup(e *longhorn.Engine, snapshotName, backupName, backupTarget,
	backingImageName, backingImageChecksum, compressionMethod string, concurrentLimit int, storageClassName, objectEndpointBackup string,
	...
	backupID, replicaAddress, err := p.grpcClient.SnapshotBackup(string(e.Spec.BackendStoreDriver), e.Name,
		...
		backingImageChecksum, compressionMethod, concurrentLimit, storageClassName, objectEndpointBackup, labels, credentialEnv,
	)
	if err != nil {
		return "", "", err
	}
