...
func (p *Proxy) ReplicaAdd(obj DataEngineObject, replicaName, replicaAddress string, restore, fastSync bool, localSync *etypes.FileLocalSync, replicaFileSyncHTTPClientTimeout, grpcTimeoutSeconds int64, linkedCloneSrcReplicaName string) (err error) {
	...
	return p.grpcClient.ReplicaAdd(obj.GetDataEngine(), obj.GetEngineName(), obj.GetEngineFrontendName(), obj.GetVolumeName(), p.DirectToURL(obj),
		...
		int(replicaFileSyncHTTPClientTimeout), fastSync, localSync, linkedCloneSrcReplicaName, grpcTimeoutSeconds)
}
