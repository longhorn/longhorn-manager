...
func (p *Proxy) ReplicaAdd(e *longhorn.Engine, replicaName, replicaAddress string, restore, fastSync bool, replicaFileSyncHTTPClientTimeout int64, localSync *etypes.FileLocalSync) (err error) {
	return p.grpcClient.ReplicaAdd(string(e.Spec.DataEngine), e.Name, e.Spec.VolumeName, p.DirectToURL(e),
		...
		int(replicaFileSyncHTTPClientTimeout), fastSync, localSync)
}
