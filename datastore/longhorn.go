...
// ListReplicasByVolume gets a list of Replicas on a specific volume the given namespace.
func (s *DataStore) ListReplicasByVolume(volume string) (map[string]*longhorn.Replica, error) {
	volumeSelector, err := metav1.LabelSelectorAsSelector(&metav1.LabelSelector{
		MatchLabels: map[string]string{
			types.LonghornLabelVolume: volume,
		},
	})
	if err != nil {
		return nil, err
	}
	return s.listReplicas(volumeSelector)
}
...
// ListEnginesByVolume gets a list of Engines on a specific volume the given namespace.
func (s *DataStore) ListEnginesByVolume(volume string) (map[string]*longhorn.Engine, error) {
	volumeSelector, err := metav1.LabelSelectorAsSelector(&metav1.LabelSelector{
		MatchLabels: map[string]string{
			types.LonghornLabelVolume: volume,
		},
	...
	return s.listEngines(volumeSelector)
}
...
func (s *DataStore) IsStorageSpaceSufficientForSnapshotPurge(volumeName string) (bool, error) {
	replicas, err := s.ListReplicasByVolume(volumeName)
	if err != nil {
		return false, err
	}

	disks, err := s.listDisksContainingReplicas(replicas)
	if err != nil {
		return false, errors.Wrapf(err, "failed to list disks containing replicas for volume %v", volumeName)
	}

	requiredPurgeReservedSpace, err := s.getRequiredPurgeReservedSpace(volumeName)
	if err != nil {
		return false, errors.Wrapf(err, "failed to get required reserved space for volume %v", volumeName)
	}

	// Walk through each disk containing the replica of the volume and
	// check if the disk space enough for snapshot purge.
	for diskID, disk := range disks {
		if ok, err := s.isDiskSpaceSufficientForSnapshotPurge(diskID, disk, requiredPurgeReservedSpace); !ok {
			return false, errors.Wrapf(err, "failed to check disk %v space for snapshout purge for volume %v", diskID, volumeName)
		}
	}
...
func (s *DataStore) listDisksContainingReplicas(replicas map[string]*longhorn.Replica) (map[string]*Disk, error) {
	disks := map[string]*Disk{}

	for _, replica := range replicas {
		disk, err := s.getDisk(replica.Spec.NodeID, replica.Spec.DiskID)
		if err != nil {
			return nil, err
		}
		disks[replica.Spec.DiskID] = disk
	}
	return disks, nil
}

// Walk through the engines of the given volume and find the largest snapshot size which
// is regarded as the required reserved space for the volume to be purged.
func (s *DataStore) getRequiredPurgeReservedSpace(volumeName string) (int64, error) {
	engines, err := s.ListEnginesByVolume(volumeName)
	if err != nil {
		return 0, err
	}

	largestSnapshotSize := int64(0)
	for _, engine := range engines {
		size, err := getLargestSnapshotSize(engine)
		if err != nil {
			return 0, err
		}
		largestSnapshotSize = util.Max(largestSnapshotSize, size)
	}

	return largestSnapshotSize, nil
}

func (s *DataStore) isDiskSpaceSufficientForSnapshotPurge(diskID string, disk *Disk, requiredPurgeReservedSpace int64) (bool, error) {
	existingPurgeReservedSize, err := s.countExistingPurgeReservedSizeOnDisk(diskID)
	if err != nil {
		return false, err
	}

	return isDiskSpaceSufficient(disk, existingPurgeReservedSize, requiredPurgeReservedSpace), nil
}

func isDiskSpaceSufficient(disk *Disk, existingPurgeReservedSize, requiredPurgeReservedSpace int64) bool {
	return (disk.status.StorageAvailable - disk.spec.StorageReserved) >=
		(existingPurgeReservedSize + requiredPurgeReservedSpace)
}

// Walk through each replica on the given disk and find out the largest snapshot
// size of each replica being purging. The size is regarded as the reserved size
// for the replica.
// Total reserved size on the disk is the sum of all reserved sizes.
func (s *DataStore) countExistingPurgeReservedSizeOnDisk(diskID string) (int64, error) {
	totalReservedSize := int64(0)

	replicas, err := s.ListReplicasByDiskUUID(diskID)
	if err != nil {
		return 0, err
	}

	for _, replica := range replicas {
		volume, err := s.GetVolume(replica.Spec.VolumeName)
		if err != nil {
			return 0, err
		}

		engines, err := s.ListEnginesByVolume(replica.Spec.VolumeName)
		if err != nil {
			return 0, err
		}
		for _, engine := range engines {
			if !isSnapshotPurging(engine.Status.PurgeStatus) {
				continue
			}

			largestSnapshotSize, err := getLargestSnapshotSize(engine)
			if err != nil {
				return 0, err
			}

			totalReservedSize += util.Min(volume.Spec.Size, largestSnapshotSize)
		}
	}

	return totalReservedSize, nil
}

func getLargestSnapshotSize(engine *longhorn.Engine) (int64, error) {
	size := int64(0)
	for _, snapshotInfo := range engine.Status.Snapshots {
		...
		size = util.Max(size, snapshotSize)
	}

	return size, nil
}

func isSnapshotPurging(purgeStatus map[string]*longhorn.PurgeStatus) bool {
...
func (s *DataStore) getDisk(nodeID, diskID string) (*Disk, error) {
	node, err := s.GetNode(nodeID)
	if err != nil {
		return nil, err
	}
	for diskName, diskStatus := range node.Status.DiskStatus {
		if diskStatus.DiskUUID != diskID {
			continue
		}

		diskSpec, ok := node.Spec.Disks[diskName]
		if !ok {
			return nil, fmt.Errorf("disk spec does not exist for node %v disk uuid %v", nodeID, diskID)
		}

		return &Disk{
			spec:   diskSpec.DeepCopy(),
			status: diskStatus.DeepCopy(),
		}, nil
	}

	return nil, fmt.Errorf("disk status does not exist for node %v disk uuid %v", nodeID, diskID)
}
