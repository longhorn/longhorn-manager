...
// CreateStorageClass creates StorageClass with the given name
...
// This function returns direct reference to the internal cache object and should not be mutated.
// Consider using this function when you can guarantee read only access and don't want the overhead of deep copies
func (s *DataStore) GetConfigMapRO(cfmName string) (*corev1.ConfigMap, error) {
	return s.cfmLister.ConfigMaps(s.namespace).Get(cfmName)
}
