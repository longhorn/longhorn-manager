...
func NewDataStore(namespace string, lhClient lhclientset.Interface, kubeClient clientset.Interface, extensionsClient apiextensionsclientset.Interface, informerFactories *util.InformerFactories) *DataStore {
	...
	lhOSInformer := lhInformerFactory.Longhorn().V1beta2().ObjectStores()
	cacheSyncs = append(cacheSyncs, lhOSInformer.Informer().HasSynced)
	...
	return &DataStore{
		...
		osLister:                       lhOSInformer.Lister(),
		ObjectStoreInformer:            lhOSInformer.Informer(),
