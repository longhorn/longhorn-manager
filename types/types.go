...
func GetOrphanLabelsForOrphanedDirectory(nodeID, diskUUID string) map[string]string {
	...
	labels[GetLonghornLabelKey(LonghornLabelOrphanType)] = string(longhorn.OrphanTypeReplicaDataStore)
	return labels
}
...
func GetOrphanChecksumNameForOrphanedInstance(instanceName, nodeID, dataEngineType string) string {
	return orphanPrefix + util.GetStringChecksumSHA256(strings.TrimSpace(fmt.Sprintf("%s-%s-%s", instanceName, nodeID, dataEngineType)))
}
