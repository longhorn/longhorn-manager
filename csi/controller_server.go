...
func (cs *ControllerServer) getAccessibleTopologyFromRequirements(accessibilityReqs *csi.TopologyRequirement) []*csi.Topology {
	...
	if len(accessibilityReqs.Preferred) > 0 {
		...
		logrus.Debugf("Using Preferred topology from AccessibilityRequirements: %+v", accessibleTopology)
		...
		logrus.Debugf("Using Requisite topology from AccessibilityRequirements: %+v", accessibleTopology)
	}
