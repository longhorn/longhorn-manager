...
func (cs *ControllerServer) createCSISnapshotTypeLonghornBackup(req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	if backup != nil {
		...
		rsp := createSnapshotResponse(backup.VolumeName, snapshotID, backup.SnapshotCreated, backup.VolumeSize,
			backup.State == string(longhorn.BackupStateCompleted))
		return rsp, nil
	}
	...
	logrus.Infof("createCSISnapshotTypeLonghornBackup: volume %s backup %s of snapshot %s created", existVol.Name,
		backup.Id, csiSnapshotName)
	snapshotID := encodeSnapshotID(csiSnapshotTypeLonghornBackup, existVol.Name, backup.Id)
	rsp := createSnapshotResponse(existVol.Name, snapshotID, snapshot.Created, existVol.Size,
		backup.State == string(longhorn.BackupStateCompleted))
	return rsp, nil
}
