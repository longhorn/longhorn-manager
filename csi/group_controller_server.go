...
func (srv *GroupControllerServer) GetVolumeGroupSnapshot(ctx context.Context, req *csi.GetVolumeGroupSnapshotRequest) (*csi.GetVolumeGroupSnapshotResponse, error) {
	...
	if csiSnapshotType == csiSnapshotTypeLonghornBackup {
		...
		// The sidecar calls Create only until the first response records the
		// handle and polls through Get from then on, so Get drives the same
		// fan-out: it recreates missing member backups, surfaces failed
		// uploads, and stamps completion. The backup mode comes from the
		// parameters recorded at group creation.
...
// backups-completed annotation once every upload completes. The sidecar calls
// Create only until the first response records the handle and polls through
// Get afterward, so both RPCs drive this same fan-out; every call is
// stateless and idempotent. A recorded group is returned as is, and a member
// backup deleted after the stamp is not recreated. The fan-out only runs on
// a Ready group: a statically provisioned content can poll a group that is
// still taking snapshots or has failed.
