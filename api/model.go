	...
	for name, disk := range node.Spec.Disks {
		...
		if diskStatus != nil {
			di.DiskStatus = DiskStatus{
				Conditions:                sliceToMap(diskStatus.Conditions),
				...
				HealthData:                diskStatus.HealthData,
				HealthDataLastCollectedAt: diskStatus.HealthDataLastCollectedAt.String(),
			}
		}
		disks[name] = di
	}
