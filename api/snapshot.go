...
func (s *Server) SnapshotBackup(w http.ResponseWriter, req *http.Request) (err error) {
	...
	if err := s.m.BackupSnapshot(bsutil.GenerateName("backup"), vol.Spec.BackupTargetName, volName, input.Name, labels, input.BackupMode, input.CleanupBackupSnapshot); err != nil {
		return err
	}
