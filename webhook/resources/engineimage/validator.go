...
func (e *engineImageValidator) Delete(request *admission.Request, obj runtime.Object) error {
	...
	if ei.Status.RefCount != 0 {
		return werror.NewInvalidError(fmt.Sprintf("deleting engine image %v (%v) is not allowed while it is being used by %d volumes", ei.Name, ei.Spec.Image, ei.Status.RefCount), "")
	}
