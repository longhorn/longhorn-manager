...
func (v *shardValidator) Create(request *admission.Request, newObj runtime.Object) error {
	...
	// The CRD's Minimum=0 marker already rejects a negative SlotIndex before admission,
	// so only the upper bound is checked here.
	if shard.Spec.SlotIndex > maxSlotIndex {
...
func (v *shardValidator) Delete(request *admission.Request, obj runtime.Object) error {
	...
	if err != nil {
		...
		// A non-NotFound lookup failure is a transient cache/API error, not a bad spec;
		// keep it retryable, matching Create.
		return werror.NewInternalError(fmt.Sprintf("failed to get ShardGroup %v: %v", shard.Spec.ShardGroupName, err))
	}
	...
	// An EC rebuild reconstructs a lost shard from the surviving healthy shards, so at
	// least DataChunks (k) must stay healthy. Block a delete only when the target is
	// healthy and removing it would leave fewer than k healthy shards. Deleting a Failed
	// or Replacing shard is always allowed: it is not a rebuild source, and blocking it
	// would stop the controller from re-provisioning a failed slot.
	healthyPeers := 0
	for _, s := range shards {
	...
	if shard.Status.State == longhorn.ShardStateNormal && healthyPeers < sg.Spec.DataChunks {
		return werror.NewInvalidError(
			fmt.Sprintf("cannot delete healthy shard %v: only %v healthy shard(s) would remain, below the %v (DataChunks) required as EC rebuild sources; repair a failed slot first to raise the healthy count, then retry",
				shard.Name, healthyPeers, sg.Spec.DataChunks),
			"")
	}
