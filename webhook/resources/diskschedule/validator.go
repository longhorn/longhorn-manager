...
func (v *diskScheduleValidator) Create(request *admission.Request, newObj runtime.Object) error {
	...
	if err := validateResourceRequirement(diskSchedule.Spec.SchedulingReplicas); nil != err {
		return werror.NewInvalidError(fmt.Sprintf("invalid scheduling replicas on disk %v: %v", diskSchedule.Name, err.Error()), "diskSchedule.Spec.SchedulingReplicas")
	}
	if err := validateResourceRequirement(diskSchedule.Spec.SchedulingBackingImages); nil != err {
		return werror.NewInvalidError(fmt.Sprintf("invalid scheduling backing images on disk %v: %v", diskSchedule.Name, err.Error()), "diskSchedule.Spec.SchedulingBackingImages")
	}
...
func (v *diskScheduleValidator) Update(request *admission.Request, oldObj runtime.Object, newObj runtime.Object) error {
	...
	if err := validateResourceRequirement(newDiskSchedule.Spec.SchedulingReplicas); nil != err {
		return werror.NewInvalidError(fmt.Sprintf("invalid scheduling replicas on disk %v: %v", newDiskSchedule.Name, err.Error()), "diskSchedule.Spec.SchedulingReplicas")
	}
	if err := validateResourceRequirement(newDiskSchedule.Spec.SchedulingBackingImages); nil != err {
		return werror.NewInvalidError(fmt.Sprintf("invalid scheduling backing images on disk %v: %v", newDiskSchedule.Name, err.Error()), "diskSchedule.Spec.SchedulingBackingImages")
	}
