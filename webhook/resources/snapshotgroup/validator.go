...
func (v *snapshotGroupValidator) Create(request *admission.Request, newObj runtime.Object) error {
	...
	if snapshotGroup.Spec.DeadlineSeconds < types.SnapshotGroupMinDeadlineSeconds ||
		snapshotGroup.Spec.DeadlineSeconds > types.SnapshotGroupMaxDeadlineSeconds {
		return werror.NewInvalidError(fmt.Sprintf("deadlineSeconds %v is outside the valid range %v to %v",
			snapshotGroup.Spec.DeadlineSeconds, types.SnapshotGroupMinDeadlineSeconds, types.SnapshotGroupMaxDeadlineSeconds), "spec.deadlineSeconds")
	}
