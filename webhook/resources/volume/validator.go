...
func (v *volumeValidator) validateExpansionSize(oldVolume *longhorn.Volume, newVolume *longhorn.Volume) error {
	...
	replicas, err := v.ds.ListVolumeReplicas(newVolume.Name)
	if err != nil {
		return err
	}
	for _, replica := range replicas {
		...
		if !datastore.IsSupportedVolumeSize(diskStatus.FSType, newSize) {
			return fmt.Errorf("file system %s does not support volume size %v", diskStatus.FSType, newSize)
		}
		break
	}
