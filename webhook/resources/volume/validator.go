...
func (v *volumeValidator) Create(request *admission.Request, newObj runtime.Object) error {
	...
	if volume.Spec.TargetNodeIDForLiveUpgrade != "" {
		return werror.NewInvalidError("spec.targetNodeIDForLiveUpgrade should be empty for a new volume", "spec.targetNodeIDForLiveUpgrade")
	}
...
func (v *volumeValidator) Update(request *admission.Request, oldObj runtime.Object, newObj runtime.Object) error {
	...
	if types.IsDataEngineV2(newVolume.Spec.DataEngine) {
		...
		if dataEngineUpgradeRequestedCondition.Status == longhorn.ConditionStatusTrue {
			...
			if oldVolume.Spec.TargetNodeIDForLiveUpgrade == "" && newVolume.Spec.TargetNodeIDForLiveUpgrade != "" {

				if newVolume.Spec.NumberOfReplicas <= 1 {
					return werror.NewInvalidError(fmt.Sprintf("unable to change targetNodeIDForLiveUpgrade for volume %v when the volume has only one replica during v2 data engine upgrade", newVolume.Name), "spec.targetNodeIDForLiveUpgrade")
				}

				// Setting targetNodeIDForLiveUpgrade to the same node is not meaningless and is not allowed
				if newVolume.Spec.TargetNodeIDForLiveUpgrade == oldVolume.Spec.NodeID {
					return werror.NewInvalidError(fmt.Sprintf("unable to change targetNodeIDForLiveUpgrade for volume %v to the same node %v during v2 data engine upgrade", newVolume.Name, newVolume.Spec.TargetNodeIDForLiveUpgrade), "spec.targetNodeIDForLiveUpgrade")
				}
			...
			if oldVolume.Spec.TargetNodeIDForLiveUpgrade != newVolume.Spec.TargetNodeIDForLiveUpgrade {
				return werror.NewInvalidError(fmt.Errorf("unable to change targetNodeIDForLiveUpgrade for volume %v when the node has not requested v2 data engine upgrade", newVolume.Name).Error(), "spec.targetNodeIDForLiveUpgrade")
			}
		...
		if newVolume.Spec.TargetNodeIDForLiveUpgrade != "" {
			return werror.NewInvalidError("unable to set targetNodeIDForLiveUpgrade for volume when the volume is not using data engine v2", "spec.targetNodeIDForLiveUpgrade")
		}
	}
