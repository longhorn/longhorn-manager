...
func (v *volumeMutator) Create(request *admission.Request, newObj runtime.Object) (admission.PatchOps, error) {
	...
	if volume.Spec.SnapshotMaxCount == 0 {
		snapshotMaxCount, err := v.getSnapshotMaxCount(volume.Spec.DataEngine)
		if err != nil {
...
func (v *volumeMutator) getSnapshotMaxCount(dataEngine longhorn.DataEngineType) (int, error) {
	c, err := v.ds.GetSettingAsIntByDataEngine(types.SettingNameSnapshotMaxCount, dataEngine)
	if err != nil {
		return 0, err
	}
	return int(c), nil
