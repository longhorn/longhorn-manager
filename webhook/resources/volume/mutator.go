...
func (v *volumeMutator) Create(request *admission.Request, newObj runtime.Object) (admission.PatchOps, error) {
	...
	if volume.Spec.FromBackup != "" {
		bName, remoteBVName, _, err := backupstore.DecodeBackupURL(volume.Spec.FromBackup)
		if err != nil {
		...
		bv, err := v.ds.GetBackupVolumeWithBackupTargetAndVolumeRO(backupTargetName, remoteBVName)
		if err != nil {
			return nil, werror.NewInvalidError(fmt.Sprintf("cannot get backup volume %s of backup target %s: %v", remoteBVName, backupTargetName, err), "")
		}
		...
		if bv != nil && backup != nil && backup.Status.VolumeBackingImageName != "" {
			...
			if volume.Spec.BackingImage == "" {
				...
				logrus.Debugf("Since the backing image is not specified during the restore, "+
					...
					bv.Status.BackingImageName, remoteBVName, name)
			}
		...
		moreLabels[types.LonghornLabelBackupVolume] = remoteBVName
	}
	if backupTargetName == "" {
