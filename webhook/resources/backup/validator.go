...
func (b *backupValidator) Create(request *admission.Request, newObj runtime.Object) error {
	...
	if backup.Spec.SnapshotName != "" {
		...
		if backup.Spec.BackupTargetName != volume.Spec.BackupTargetName {
			return werror.NewInvalidError(fmt.Sprintf("failed to create the backup %v due to a mismatch between the assigned backup target and the backup target of the volume %v (%v: %v)", backup.Name, volume.Name, backup.Spec.BackupTargetName, volume.Spec.BackupTargetName), "")
		}
		...
		if backupTarget.Spec.ReadOnly {
			return werror.NewInvalidError(fmt.Sprintf("failed to create the backup %v on a read-only backup target %v ", backup.Name, backupTarget.Name), "")
		}
	}
