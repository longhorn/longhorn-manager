...
func (b *backupValidator) Create(request *admission.Request, newObj runtime.Object) error {
	...
	if err := b.validateBackupBlockSize(backup); err != nil {
...
func (b *backupValidator) Update(request *admission.Request, oldObj runtime.Object, newObj runtime.Object) error {
	...
	isValidOldBackupBlockSize := b.validateBackupBlockSize(oldBackup) == nil
	...
	if err := b.validateBackupBlockSize(newBackup); err != nil {
		return werror.NewInvalidError(err.Error(), "")
	}
...
func (b *backupValidator) validateBackupBlockSize(backup *longhorn.Backup) error {
	blockSize, err := util.ConvertSize(backup.Spec.BackupBlockSize)
	if err != nil {
		return err
	}
	...
	if backupVolumeName, exist := backup.Labels[types.LonghornLabelBackupVolume]; exist {
		...
		if err != nil {
			if !datastore.ErrorIsNotFound(err) {
				return fmt.Errorf("fail to get volume size of %s from datastore: %w", backupVolumeName, err)
			}
		} else {
			volumeSize = volume.Spec.Size
		}
	}
	return types.ValidateBackupBlockSize(volumeSize, blockSize)
}
