	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				return client.LonghornV1alpha1().Settings(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				return client.LonghornV1alpha1().Settings(namespace).Watch(options)
			},
		},
		&longhorn_v1alpha1.Setting{},
		resyncPeriod,
		indexers,
	)
}
...
func defaultSettingInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewSettingInformer(client, v1.NamespaceAll, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
}

func (f *settingInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&longhorn_v1alpha1.Setting{}, defaultSettingInformer)
}
