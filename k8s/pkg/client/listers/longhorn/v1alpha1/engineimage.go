...
type EngineImageLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.EngineImage, err error)
...
// List lists all EngineImages in the indexer.
func (s *engineImageLister) List(selector labels.Selector) (ret []*v1alpha1.EngineImage, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.EngineImage))
	})
	return ret, err
}
...
type EngineImageNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.EngineImage, error)
...
// List lists all EngineImages in the indexer for a given namespace.
func (s engineImageNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.EngineImage, err error) {
...
// Get retrieves the EngineImage from the indexer for a given namespace and name.
func (s engineImageNamespaceLister) Get(name string) (*v1alpha1.EngineImage, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("engineimage"), name)
	}
	return obj.(*v1alpha1.EngineImage), nil
}
