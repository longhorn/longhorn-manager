...
// Backend represents one peer SPDK process - a replica or a shardgroup -
...
// directly without going through Backend. Adding a Delete method here would
...
// backend-aware cleanup needs to add the method deliberately and review the
// blast radius.
type Backend interface {
	// Identity (immutable for the life of the backend).
	Name() string
	Address() string
	...
	// Get returns a snapshot of the backend's current state. For
	// replicaBackend this issues ReplicaGet; for shardGroupBackend it
	// issues ShardGroupGet (the engine never queries individual shards).
	Get() (*BackendView, error)

	// Snapshot operations forward to the backend's snapshot RPCs.
	...
	// BackingImageGet resolves a backing-image record on the backend.
	// RAID1 dispatches to BackingImageGet on the replica's SPDK service;
	// shardGroupBackend returns (nil, nil) - EC volumes do not surface
	// backing-image state at the engine layer.
	BackingImageGet(name, lvsUUID string) (*api.BackingImage, error)
}

// BackendFactory builds an Backend for a given (name, address) pair. The
// engine receives the factory at Create time from the server layer, which
// picks newReplicaBackend or newShardGroupBackend keyed on
...
// newReplicaBackend directly and only handles RAID1; EC engines are
// rejected at the server boundary by isShardedEngine.
type BackendFactory func(name, address string) Backend

// BackendView is the engine's view of an backend's current state, returned
// by Backend.Get(). Both implementations populate the same fields from their
...
func isBackendDispatchable(u Backend) bool {
...
// GetServiceClient, logging any error. name identifies the backend.
